...
- 仓库不保留已完成 Change 或迁移前 OpenSpec 的历史目录。只有任务明确要求历史审计、恢复形成时依据或比较演进时，
  才按[项目知识与变更治理](docs/governance/knowledge-maintenance.md)从版本历史取得线索，并以当前 owner 重新核对。
...
| 明确 change 需要跨文件、owner 或验证阶段持久交接 | 使用 `change-plan` skill，先运行 `bun run change-plan -- list changes`，再读取目标 `changes/<change>/`；完成后的目录删除只在当前任务明确授权时执行。 |
...
- 项目子代理默认使用 `gpt-5.6-terra`。
- 仅在任务明确需要更高智能时使用 `gpt-5.6-sol`，并说明选择原因。
- 项目子代理不使用 `gpt-6-astra`。
