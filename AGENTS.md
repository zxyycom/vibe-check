...
- 历史材料不参与当前规范、计划或验证。只有任务明确要求历史审计、恢复形成时依据或比较演进时，
  才按[项目知识与变更治理](docs/project-knowledge-and-change-governance.md)进入归档。
...
| 形成或修改会持续影响后续行为、owner、边界、兼容性、风险处理或验收方式的判断 | 使用 `decision-records` skill，先运行 `bun run decisions -- list`；涉及 Change 交接时再读[项目知识与变更治理](docs/project-knowledge-and-change-governance.md)，维护后运行 `bun run decisions -- check`。 |
...
| 调查或修复中确认 Bug 达到项目定义的复杂或严重条件 | 不等待用户再次确认；先读[复杂或严重 Bug 的自动调查沉淀](docs/project-knowledge-and-change-governance.md#复杂或严重-bug-的自动调查沉淀)，再使用 `investigation-report` skill，在当前任务交付前创建并验证报告。 |
