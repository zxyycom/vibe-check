...
import { projectHumanStatus, type HumanStatusProjection } from "../../check-record/human-status.ts";
import type { CoreSnapshot, QualityRecord } from "../../check-record/model.ts";
import type {
  DecisionEvidence,
  EvidenceRef,
...
export function createPublicationModelV3(
  input: Readonly<{
...
function validatePublicationEvidence(
  ...
  if (
    index.referencesByName.size !== references.length ||
  ...
  if (
    index.referenceEvidencePairs.size !== referenceFacts.evidence.length ||
    referenceFacts.evidence.some(
      (evidence) =>
        !index.checkIds.has(evidence.checkId) || !index.referencesByName.has(evidence.referenceName)
    )
  ) {
...
function buildPublicationEvidenceIndex(
  ...
  return {
    ...
    referenceEvidencePairs: new Set(
      referenceFacts.evidence.map(
        (evidence) => `${evidence.checkId}\u0000${evidence.referenceName}`
      )
    ),
...
function unexpectedEvidenceReference(reference: never): never {
  throw new TypeError(
    `Unknown publication decision evidence reference: ${JSON.stringify(reference)}`
  );
}
