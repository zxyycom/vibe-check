...
export function validateMachinePublicationSetV2(input: Readonly<{
  ...
  }) satisfies MachinePublicationV2);
}
...
function parseRun(bytes: Uint8Array): Parsed<MachineRunV2> {
  ...
  if (!Value.Check(MACHINE_RUN_V2_SCHEMA, parsed.value)) {
    const error = Value.Errors(MACHINE_RUN_V2_SCHEMA, parsed.value)[0];
...
function parseRecordSegments(segments: readonly string[]): Parsed<MachineRecordV2[]> {
  ...
  for (const [index, segment] of segments.entries()) {
    ...
    if (!parsed.ok) return parsed;
    if (!Value.Check(MACHINE_RECORD_V2_SCHEMA, parsed.value)) {
      const error = Value.Errors(MACHINE_RECORD_V2_SCHEMA, parsed.value)[0];
      return validationFailure({
        category: "schema",
        index,
        line: index + 1,
