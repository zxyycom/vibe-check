...
import type { CoreCheck, CoreRecord, CoreSnapshot } from "./model.ts";
...
export function validateCheckDefinition(value: unknown): ValidationResult<CheckDefinition> {
  ...
  if (
    ...
    !isCheckTreeReferenceId(definition.checkId) ||
  ...
  return accepted(
    Object.freeze({ checkId: definition.checkId, displayName: definition.displayName })
  );
}
...
function validateReason(
  ...
  if (
    ...
    checkIds === undefined ||
    checkIds.length === 0 ||
    checkIds.some((checkId) => typeof checkId !== "string" || !isCheckTreeReferenceId(checkId))
  )
    return null;
