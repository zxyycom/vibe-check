...
import type { MachineRunV2 } from "./schema.ts";
import {
  setInvariantFailure,
...
type Decision = MachineRunV2["decision"];
...
export interface DecisionReferenceIndex {
  ...
  readonly referencesByName: ReadonlyMap<
    string,
    MachineRunV2["references"]["identities"][number]
  >;
...
function unknownDecisionReference(
  ...
  if (reference.kind === "run") {
    return index.runIds.has(reference.checkRunId) ? null : decisionReferenceFailure(
      "decision-run-reference",
      "Decision references an unknown run."
    );
  }
...
export function validateDecisionState(run: MachineRunV2): ValidationFailure | null {
...
function validateDecisionCanonicalOrder(run: MachineRunV2): ValidationFailure | null {
  const decision = run.decision;
...
export function collectDecisionReferences(run: MachineRunV2): readonly EvidenceRef[] {
  return [
