...
import { fromNullable, none, some } from "./option.ts";
...
describe("product Option", () => {
  it("composes present values without entering absence branches", () => {
    ...
    assert.equal(present.andThen((value) => some(`value:${value}`)).unwrapOr("missing"), "value:2");
    assert.strictEqual(
      present.filter((value) => value > 1),
      present
    );
    assert.strictEqual(
      present.filter((value) => value > 2),
      none
    );
    assert.equal(
      present.match(
