...
describe("Lizard target files", () => {
  it("selects every Lizard 1.23-supported extension case-insensitively and excludes fallback inputs", () => {
    const supported = (
      "c cc cjs cpp cs cxx erl es escript f f03 f08 f70 f90 f95 for fpp ftn gd go h hpp " +
      "hrl java js jsx kt kts lua m mjs mm pck php pkb pks pl plb pls pm py r rb rs scala " +
      "sol sql st swift ts ttcn ttcnpp tsx vue zig"
    )
      .split(" ")
