...
export class GoLikeStates extends CodeStateMachine {
  ...
  private readonly _expect_function_dec = (token: string): void => {
    if (token === "(") {
      ...
      this.next(this._generalize, token);
    } else {
      this.next(this.globalState);
    }
  };

  private readonly _generalize = (token: string): void => {
    this.readInsideBracketsThen("<>", token, this.ignoreToken, this._expect_function_dec);
  };
