...
describe("check-record foundation model", () => {
  it("accepts exactly one four-state terminal outcome for each Core Check", () => {
    const outcomes = [
      ...
      {
        status: "unavailable",
        reason: { code: "prerequisite-unavailable", checkIds: ["file-metrics"] }
      }
    ] as const;
  ...
  it("accepts only closed Check definitions", () => {
    assert.deepEqual(validateCheckDescriptor(definition), {
      ok: true,
      value: definition
    });
    assert.equal(validateCheckDescriptor({ ...definition, recordTypes: [] }).ok, false);
    assert.equal(
      validateCheckDescriptor({ checkId: "invalid id", displayName: "Invalid" }).ok,
      true
    );
  });
});
