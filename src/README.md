...
This `src/` directory is the Vibe Check-owned product source. This file preserves the
...
Unless a sentence explicitly marks a current location, the paths and module
names below describe the formation-time lift. They do not describe the current
source layout.
...
- Consumer repository revision
  ...
  test configuration, was placed at `quality-core/bun-test.d.ts` as test-only
  support during the initial lift. Its current location is
  `src/bun-test.d.ts`.
- quality-core revision
  `3acea8c2f643ea86f7a1e8f2a6db716b7e320c76` supplied its complete
  `src/**` and `test/**` trees. The initial lift placed them under
  `quality-core/`.
- foundation revision
...
- parallel-task-runner revision
  ...
  current `src/scheduler/**` module retains those files or their bytes.
