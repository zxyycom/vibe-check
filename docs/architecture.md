...
effective diagnostic logging enabled 时，Scheduler shell 才创建 invocation-local accumulator，并只通过 private clock/logger handoff 进行采样；disabled 时不创建它，也不为此新增 clock read。pure `decideScheduler`、custom admission callback 与 hard guard 仍不接收或读取 clock、logger 或 accumulator，custom callback 也没有 per-policy timing。shell 在 graph-ready、admit、pending/running settle、accepted explicit policy `wait` 与 terminal 等 state boundary 先安全 sample 并把旧 state interval flush，再变更真实 Scheduler state，之后只 capture 该唯一 state 的新投影；accumulator 不建立第二套 pending/running/settlement truth。clock throw、non-finite/backward sample、negative/invalid interval 或 integral 只使本次 timing projection 以稳定 reason `unavailable`，不改变 admission、cancellation、policy-fault drain 或 settlement；合法 zero span 与该状态不同，且离散 admitted/settled/max-running/wait facts 仍可形成。
...
Task 在无 directed relation 时于 Scheduler span start graph-ready，否则在全部当前 directed relation settled 时 graph-ready；只有实际 admitted Task 才有 `admissionDelayMs` 与 `taskActiveMs`。summary 至多列出三项 admission delay（delay 降序、Task ID 升序）及最后 settled Task ID 和从最后 admission 到 terminal 的 completion tail；tail 不是 critical path。仅 accepted proposal `kind: "wait"` 形成 accepted wait interval，`proposal: null` 的被动 running drain 不计 wait。control path、slot/capacity integral、wait、active duration、delay 和 tail 可以重叠，绝不可相加重建 wall time。

只要已经进入 Scheduler，normal completion、caller cancellation 与 admission-policy fault 的 drain 都在 terminal path 尝试一次有界 `[SCHEDULER] [SUMMARY] scheduler.summary` human observation；writer failure 保持 observational containment。pre-work 或 planning failure 没有 Scheduler execution，因而不伪造 summary。summary 是 private human diagnostic：不进入 public `RunResult`、Core/Check/Record facts、machine publication、progress、warning、autotune、parser/schema/version 或跨 invocation telemetry。future fail-fast 或 named-resource capacity 若改变 hard guard/capacity model，必须重新审阅这些 boundary、denominator、wait facts 和 summary。
