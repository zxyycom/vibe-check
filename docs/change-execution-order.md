...
| 1F：已验收并归档 | [`compare-lizard-python-typescript-performance`](../changes/archive/compare-lizard-python-typescript-performance/proposal.md) | 已形成迁移前 Python Product path、固定 1.24 analyzer-only 与当前 Product 的分层可重复比较 | 后续优化使用独立 Change；不得把 analyzer-only 结果外推为完整 Product regression |
| 1G：已验收并归档 | [`optimize-lizard-reader-resolution`](../changes/archive/optimize-lizard-reader-resolution/proposal.md) | 已在手写 private port façade 实施可回退 reader fast path，并闭合 filename differential、oracle 与 before/after evidence | 后续性能工作不得借此修改 source-aligned registry/core/readers/extensions；真实 Product corpus 与 core hotspot 分别形成新证据边界 |
