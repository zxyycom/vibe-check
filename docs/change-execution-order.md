...
- `5ff3149e689ad0cef789956bfab0de2baa8adf5a` 的固定 Lizard 1.24.0 source-aligned baseline、oracle、provenance、法律材料与归档结果；
...
| 1F：性能证据 | [`compare-lizard-python-typescript-performance`](../changes/compare-lizard-python-typescript-performance/proposal.md) | 建立迁移前 Python Product path、固定 1.24 analyzer-only 与当前 Product 分层的可重复比较；先测量，不预设优化 | 可与 extension consumer 选择并行；只写 benchmark/evidence owner，不修改 analyzer runtime 或把 Python 带回 Product/package |
...
| [`compare-lizard-python-typescript-performance`](../changes/compare-lizard-python-typescript-performance/proposal.md) | active Plan | 只建立和运行可重复 comparative benchmark、分层 profile 与观察性 evidence；若发现实质回归，再以独立后续 Change 决定优化 | 以当前 1.24 stable commit 为基线；可与 extension 选择证据并行，但不得与 analyzer runtime 修改并行，也不建立跨主机 timing hard Gate。 |

`sync-lizard-typescript-port-to-1-24-0` 已独立完成并归档。后续可收敛 adoption 的消费者选择，并独立核对 Python-to-TypeScript hard cut 的性能结果；只有选择一个具体 closed capability 且获得当时实施授权后，`adopt-selected-lizard-extensions` 才从包含该归档结果的 1.24 stable commit 建立实施基线。同步、性能比较与 extension adoption 不得混合为一次上游同步、隐式优化或自动 Product 扩张。
