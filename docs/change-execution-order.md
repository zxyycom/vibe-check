...
| 1C：Admission simulation Plan 已验收 | [`provide-admission-strategy-simulation`](../changes/provide-admission-strategy-simulation/proposal.md) | standalone 与 callback 的 immutable `AdmissionGraph` / `AdmissionState` 已在同一 private compiled machine、pure core state/reducer 与 canonical effects 上完成实施/验证；稳定 public contract 由 runtime、Architecture、Configuration、API mechanics 和 active + aligned [Decision](decisions/provide-immutable-admission-graph-state.md)承接。Change 保持 active Plan，直到另行完成归档。 | 已交付 custom lifecycle 仍是 stable baseline；algorithm 可复用 private core/test harness，但不依赖 simulation public contract，也不与该 active Plan 混合实现。 |
