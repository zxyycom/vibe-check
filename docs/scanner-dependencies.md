...
canonical immutable authored snapshot；每个 Check 获 Scheduler admission 后，Run 才在其 scanner 或 author callback work 前执行
task-local preflight；malformed/unknown/incomplete replacement 只把 owning Check 结算为
