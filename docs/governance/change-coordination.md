...
当前 learned 相关协调以 [公共策略重构](../../changes/archive/expose-learned-admission-strategy/proposal.md) 和
...
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1A：统一 lifecycle 基线              | [active + aligned 统一 lifecycle Decision](../decisions/keep-invocation-lifecycle-free-of-learned-special-cases.md)                                       | 当前 stable baseline 是统一 Invocation 策略生命周期与 Scheduler execution boundary；learned helper 使用相同公共 prepared 接口；runtime、Architecture 和 API mechanics 是 current owner。形成时 archive 只保留 provenance。                                                                                                                                                                                            | duration-model、task-scheduler、provider、resolved-checks 与 invocation 的 current owner 以 stable owner 为准；本行不授予新的 implementation 工作。                                                             |
...
Scheduler 轨道的 current baseline 由 runtime、Architecture、API mechanics 与 [active + aligned 统一 lifecycle Decision](../decisions/keep-invocation-lifecycle-free-of-learned-special-cases.md)承接。archive 与既有 Gate evidence 只提供形成时 provenance；它们不扩展当前 implementation authorization。
