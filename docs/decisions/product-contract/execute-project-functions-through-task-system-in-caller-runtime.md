...
relations:
  - type: 替代
    target: product-contract/contain-project-code-behind-private-runtime-boundary.md
---

## 目的

- 让项目配置中的 custom runner、TaskPlan factory 和 closures 能被 Package Run 直接调用，并复用现有 Check/Record 与 shared scheduler owners。
...
- 采用: Package Run 在调用方 Bun runtime 中验证传入 definition，并直接调用其中明确 slot 的 custom runner 与 TaskPlan factory；不序列化、反编译或跨 runtime 重新建立函数。
- 采用: Custom Check 在 planning 阶段贡献完整静态 TaskPlan，由 invocation-scoped shared scheduler 统一实施 task dependency、bounded parallelism 与 named resource coordination；执行中不得动态注册 Check 或 Task。
