...
- 采用：首次公开发布前完成并验证四项 Product-provided ordinary format Check values（`jsonValidation`、`jsonSchemaValidation`、`markdownStructureValidation`、`markdownLinkValidation`）以及唯一专用 public constructor `maintenanceReminders(entries)`；不把它伪装为第五个无参 default value。
