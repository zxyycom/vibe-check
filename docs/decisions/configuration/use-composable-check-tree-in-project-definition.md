...
- 让项目直接以 exported built-in descriptor 或 custom leaf 组成所需 Checks，而不重复 built-in catalog、selected list 或空 schedule entries。
...
- 采用: 内置 descriptor 与 project custom Check 都以 leaf 形式进入同一 tree。每个 leaf 在 validation 前后均拥有唯一稳定 Check identity；同一 resolved Check identity 不能出现两次，也不能由 custom leaf 覆盖内置 identity。
