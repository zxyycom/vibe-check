...
## 目的
- 让 installed consumer 能确定性地提供过渡阶段所需的 Python、`scc` 等外部程序，而不把 scanner implementation identity、native flags 或输出格式变成 Product-provided Check 的产品语义。

## 背景
- Tool neutrality 的目标是稳定 Product-provided Check policy、metadata 和含义，不是隐藏开源实现，也不是禁止 runtime 使用或公开说明具体工具。
...
- 外部 binding 是 `0.0.x` 形成阶段的过渡实现。后续能力可以逐步内化，但当前 package 不能通过静默缺失或缩减主要 Product-provided capability 来规避 dependency closure。
...
- 采用: Product-provided Check 的 policy fields、metadata、identity 和结果含义不按 `scc`、Lizard、jscpd 或其它 backend 的名称、native flags、exit code 或输出格式塑形；源码和文档可以如实说明实际 implementation。
- 采用: 本记录中的 Product-provided 只描述 Product 对默认 implementation 的 ownership；所有 Check 继续使用同一 construction/binding handoff，不形成另一种 public Check type、tree semantics 或 runtime binding protocol。
...
- 不采用: 在 Product-provided Check policy 中公开 scanner-native thresholds、flags 或 commands，或把 dependency discovery 留给未声明的开发环境。
