...
decision: 以 Lizard 1.23 为 oracle 维持 source-aligned analyzer closure，并以无外部 runtime 的 hard cut 作为当前产品方向。
tags:
  - configuration
  - dependency-policy
  - product-contract
...
- 采用: 以 pinned Lizard 1.23 source revision 和当前 public `functionMetrics` contract 作为首轮迁移 oracle，覆盖全部当前 reader 与 extension，不把 1.24 或后续升级混入首次 backend hard cut。
