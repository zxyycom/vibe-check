...
- 采用：Project Gate adapter 只把 invocation、configuration/run/effect facts 与 aggregate 映射为 process exit `0`、`1` 或 `2`，并保留日志责任；它不选择已退休的评估器、重算 snapshot aggregate 或解释 Check-local data。
