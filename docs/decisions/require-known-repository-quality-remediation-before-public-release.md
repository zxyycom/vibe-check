...
- 用户原始运行形成的初始快照为 27 file-metrics findings、129 function-metrics findings 和 2 Markdown link findings；本 Change 最终真实 Gate evidence 为 28、134、2。两者都是特定 invocation 的证据快照，不是稳定 policy 的配额。本轮只记录这些事实并迁移 Gate policy，不修复两条断链，也不宣称任何 finding 已解决。
...
- `preserve-release-gate-readiness-with-invocation-creation-time.md` 规定公开发布前需要完整 Gate 的真实消费者证据；`publish-unscoped-vibe-check-publicly.md` 仍要求发布前重新核验 registry authority、authentication、目标 version 与外部写入授权。当前稳定 owner 尚未把初始、最终或发布时任一已知 finding 的 remediation evidence 记为已满足的 current fact。
...
- 采用: 在每类 finding 的处置路径、届时数量/身份与复验命令明确后，更新稳定 release owner 并运行相应完整 Gate/candidate verification；在此之前本记录保持 unaligned。
