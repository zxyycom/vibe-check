...
- [在 Check Task 准入后执行 preflight](run-check-preflight-with-task-admission.md) 已替代先前的 options-preflight 决策，并承接 Link-owned source selection、direct-only target resolution、root 外 `ignore | report | validate`、directory 和 GitHub-priority anchor 的长期边界。
