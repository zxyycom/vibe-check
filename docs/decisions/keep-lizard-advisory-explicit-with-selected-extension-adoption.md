...
- 让 repository-owned maintenance advisory 持续只提示 Lizard upstream stable release，并与 ordinary Product Run、default required/full Gate 及已发布 analyzer facts 隔离。
...
- 前序 advisory direction 的 1.24 baseline 将全部 19 个 legacy concrete bodies 设为 deferred/no registration。已授权 Change `adopt-selected-lizard-extensions` 只选择 `lizardcomplextags` 与 `lizardnd`；该 Change 的 implementation、Product docs、tests、oracle、performance 与 required/full acceptance 已闭合，本更新已成为 current fact，且 advisory owner 不扩大。
