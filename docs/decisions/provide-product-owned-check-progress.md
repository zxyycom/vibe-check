...
background: Product 已拥有 Check lifecycle 与 progress rendering output，TTY 动态区域还需要明确序号、stream ownership 和失败隔离。
...
- 让任何 package consumer 只需启用现有 progress rendering output，即可获得 Product 一致维护的 Check 总数、逐项完成状态、耗时和 final summary；只有 TTY target 额外显示运行中状态。
...
- Product 已拥有 canonical Check catalog、实际 execution/settlement、final outcome 和 progress rendering output；项目若另建 observer/renderer，会复制 Product 已知的 lifecycle 与 status mapping。
...
- Console output 是可关闭的 tool output，不是 quality fact。输出失败若中止 Check/Record closure，会让 presentation 成为第二个 execution control plane。

## 决策

- 采用: Product progress rendering output 通过 package-private prepared/started/settled/final feedback 驱动基础 progress；不增加 Check-owned callback、project-supplied observer、public lifecycle event 或 custom renderer API。
...
- 采用: 第一次 progress write/rewrite failure 将 progress rendering output 置为 failed 并停止后续 progress writes，但继续闭合 Task、Check、Record 和其他 enabled outputs；console failure 不得伪造或改写 execution facts。
