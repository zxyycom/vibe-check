...
relations:
  - type: 归并
    ...
    summary: 将字段 shape 收敛为 Check local data
  - type: 归并
    target: 260821-use-check-owned-extractors-for-opaque-record-identities
    summary: 以 Check local ID 取代 opaque ID 提取链
---

## 目的

- 让 Product-provided defaults 与 project-authored custom Checks 使用同一个不假设 file、finding、metric、baseline behavior 或 presentation 的 Record contract。
...
- Repository summary、API health、performance sample、dependency fact 与 file finding 的 data shape、location、severity、message 和 baseline semantics稳定不同，不能从当前 built-ins 提炼为共同 Record fields。
...
- 不采用：Record tuple generic、字段类型目录、field descriptor/Schema、`identityFields`、`identify`、Product-generated opaque ID，或把 kind、level、subject、message、location 和 baseline semantics加入 base Record。
