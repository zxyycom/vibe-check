...
- npm package 能承接 TypeScript 产品的安装、版本锁定、声明与资源分发；采用 npm 作为分发载体不自动决定宿主 runtime、公共名称或执行界面。
...
- 采用: Package 必须声明并验收实际支持的宿主 runtime、平台前提与 production dependency closure；使用 npm 分发本身不能代替宿主验收。
