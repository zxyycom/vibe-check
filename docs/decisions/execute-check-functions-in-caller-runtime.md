...
background: Functions 已随项目 import 存在于调用方 runtime；跨进程重载会复制配置和改变 closure 语义。
...
- Project Definition 已由项目运行入口普通 import，functions 已存在于调用方 runtime。
...
- 采用: Package Run 在调用方 runtime 中验证 definition，并把每个 execution-bearing Check function 投影为 shared scheduler 管理的独立 Product Task；具体受支持宿主由独立 package-host Decision 承接。
