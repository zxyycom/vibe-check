...
relations:
  - type: 修订
    ...
    summary: 在产品自有 analyzer 中私有采用指定 Lizard 扩展
---
...
- 采用: 仅在独立 Change 完成 source-range/hash/SPDX/provenance、translated target、processor/lifecycle、27-reader/55-suffix oracle、resource/performance、Product docs/tests 与 workspace acceptance 后，私有采用 `complextags` 与 ND。
- 采用: `complextags` 只为 CCN-over-limit Finding 的 Record 增加完整有序 token/line contributors 和 bounded human explanation，不新增 metric、limit、waiver identity、settlement 或 final-data field。
- 采用: ND 是默认 `7` 的 `nestingDepth.maximum` closed metric，有独立 Finding/waiver/Record/message/final-data contract，并保留 `?` 与 condition 中第一个 `&&`/`||` 的 ND source semantics。
