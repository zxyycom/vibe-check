...
- 采用: translated core/readers/shared/extensions 的 source fidelity 优先于仅为项目命名、函数/文件长度、complexity、duplicate detection 或统一抽象而重构的压力。
- 采用: 适用的 development 与 Gate quality owner 实现 evidence-qualified profile。只有 source-alignment evidence 支持、范围最小且带 upstream-sync review trigger 的 lint、format 或 Gate path/rule exception 才可保留。
- 采用: 当前精确 selection、计数和机械验证属于 stable tooling docs 与其 tests，不由本 Decision 固化。
- 采用: profile 不得排除 TypeScript parse/type/build、运行时、Lizard oracle/parity、processor/extension lifecycle、source identity、deviation mapping、provenance/license 或 import/public-surface boundary 验证。
- 采用: 手写 port façade、Product adapter、Worker、Check 与所有 tests 不继承 translated-source exception，继续遵守各自适用的普通项目检查；整目录免检无效。
