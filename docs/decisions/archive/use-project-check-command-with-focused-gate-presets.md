...
id: 260904-use-project-check-command-with-focused-gate-presets
status: archived
alignment: aligned
...
- Product 的 `enabledByFlags` 能在 author work 前统一结算未选择 Check，但不会自动选择 `dependsOn` 闭包。当前项目可以为有限 preset 显式维护闭包；通用传递能力曾由形成时的 Draft Change `propagate-flag-selection-through-check-dependencies` 评估，该已删除材料不授权本轮实施 Product API。
