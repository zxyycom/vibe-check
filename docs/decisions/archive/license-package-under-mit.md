...
decision: "`@zxyycom/vibe-check` package 使用 SPDX `MIT`；candidate 和发布材料必须包含匹配的许可文本及权利人信息。"
tags:
  - product-contract
relations: []
---
...
## 目的
- 让 `@zxyycom/vibe-check` 的公开安装、使用、复制和分发具有明确且一致的许可表达，并让 package materials 可以被自动化与消费者核验。
...
## 决策
- 采用: `@zxyycom/vibe-check` candidate manifest 和 published package 使用 SPDX expression `MIT`。
