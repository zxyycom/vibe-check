...
- 让一个已导入的 Project Definition value 完整拥有 recursive Checks、每项 execution configuration、调度和 outputs。
...
- project root、changed files、flags、显式 aggregation、signal 和 invocation output overrides 只在特定 Run 中存在，仍适合作为 closed Run Controls。

## 决策

- 采用: Project Definition value 是产品执行语义的主要公开配置输入，拥有 recursive Check declarations、每个 Check 的完整 options、scheduler 与 outputs。
...
- 采用: Run Controls 只表达 shared invocation input，例如 project root、changed files、flags、显式 aggregation、signal 和受支持的 output override；它不能注册 Check、改写 Check options、覆盖 external executable 或增加 execution variant。
