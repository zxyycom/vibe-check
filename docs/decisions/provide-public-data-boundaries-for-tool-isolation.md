...
- 采用: 公开最小、可被外部用户独立调用的数据契约，以 JSON 规范化、确定性序列化和外层配置快照任务审查公开面。它们可以作为公开 Core tool 复用共同规则与常用组合，不要求每个便利变体独立承接完整业务流程；但便利必须对外部调用方成立，不能只为内部工具绕过边界。不机械公开整个私有模块，也不要求用户掌握内部接线。
