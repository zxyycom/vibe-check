...
background: 私有 workload 不可运行；repository-owned deterministic corpus 已通过 persistent 和 memo 采用门槛。调用方确认它仅为显式启用的性能工具。
...
- repository-owned deterministic corpus 先以 prototype 选择 persistent + memo，随后由实际 runtime 的五组交错 retest 验收：cold `21.74%` faster、warm `61.61%`、single-file incremental `61.78%`；warm/incremental 分别节省 `1925.18 ms` / `1972.73 ms`。formal runtime 不公开 physical read/parse counters，target memo 的 scoped reuse 由直接 resolver test 证明。原始样本、环境、cache footprint 和限制由 Change evidence owner 保存。
