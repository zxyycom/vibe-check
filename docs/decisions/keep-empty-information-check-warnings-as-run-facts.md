...
purpose: 让合法空 information Check 的非阻断 warning 只作为所有 RunResult 分支的结构化事实，而不恢复 Product warning 输出 effect。
background: 空 Check warning 已由 Definition normalization 形成；Product 只有 cache、progress、output effects。
...
- Package Run 在配置、planning、cancelled、execution、effect 与 completed result 分支都保留 `definitionWarnings`，使调用方不需要依赖终端文本恢复该事实。
- Product effects 只有 cache、progress 与 output。progress 只使用 Run-owned Check lifecycle、duration 和受控 reason code，不是 generic definition-warning renderer；项目自己的 Gate transcript 也不构成 Product logs effect。
...
- 采用：Product 不恢复 logs effect、warning renderer、warning-specific output channel 或从 warning 推导 Check terminal status。progress 只负责 Check lifecycle presentation。
