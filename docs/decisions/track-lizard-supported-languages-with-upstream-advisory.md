...
background: 首轮 baseline 固定为 Lizard 1.23；已实现的 explicit-only advisory 发现上游变化，但不自动采用。
...
- 当前 analyzer baseline 固定于 Lizard 1.23.0 的 27 readers 和 55 extensions；完整 source/package/Gate 验证已使本记录的方向对齐。任何升级仍须重新核对 mapping 与 fixtures。
...
- 采用: 首轮 hard cut 保持 Lizard 1.23 的完整 enabled reader/extension surface，并同时保留完整 internal extension protocol；19 个 deferred optional concrete extension body 默认不启用。之后继续把 Lizard stable releases 作为 analyzer 演进输入，而不是永久冻结或只维护高使用率语言。
