...
id: "260825-markdown-link-validation-library-strategy"
...
**项目事实。** 形成时读取了已完成 Change `add-markdown-link-validation` 的 `proposal.md`、`design.md` 及形成时 Decision [`define-offline-markdown-link-target-boundaries.md`](../decisions/archive/define-offline-markdown-link-target-boundaries.md)。前两份 Change 材料现已按项目策略删除；这里仅记录其形成时作用，不提供当前可读取依赖。现有 `scripts/validation/links.ts` 只作为该 Change 已明确排除的 repository-local 对照，不作为候选实现。
