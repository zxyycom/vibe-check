...
| [`admission-core.ts`](../../src/project-run/task-scheduler/admission-core.ts)、[`graph-validation.ts`](../../src/project-run/task-scheduler/graph-validation.ts)、[`scheduler.ts`](../../src/project-run/task-scheduler/scheduler.ts) 与 [`execution-state.ts`](../../src/project-run/task-scheduler/execution-state.ts) | 形成时的 selection、state、settlement、compile 和 Scheduler owner 分析 | 支持本文的静态控制流与渐近推导；不替代新的 profile 或多规模测量。 |
