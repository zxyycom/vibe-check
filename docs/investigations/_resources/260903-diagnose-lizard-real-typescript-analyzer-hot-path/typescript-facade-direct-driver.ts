...
if(selected==="fastpath-count"){
 ...
 assert(current());assert(direct());const rows=[] as unknown[];for(let block=1;block<=10;block++)for(const [name,fn] of (block%2?[["current",current],["direct",direct]]:[["direct",direct],["current",current]]) as const){const x=timer(fn);assert(x.value);rows.push({block,name,ms:x.ms});}
 const c=(rows as {name:string;ms:number}[]).filter(x=>x.name==="current").map(x=>x.ms),d=(rows as {name:string;ms:number}[]).filter(x=>x.name==="direct").map(x=>x.ms);console.log(JSON.stringify({status:"complete",comparison:"current-facade-vs-pre-resolved-direct-core",digest:expected,metrics:2222,rows,current:{medianMs:med(c),p90Ms:q(c,.9)},direct:{medianMs:med(d),p90Ms:q(d,.9)},medianDirectMinusCurrentMs:med(d)-med(c)}));
}
