...
import { planSccBatches } from "../../../../src/package-checks/file-metrics/scc/batch-planner.ts";
import { scanWithScc } from "../../../../src/package-checks/file-metrics/scc/scanner.ts";
...
type Condition = {
  readonly id: string;
  readonly kind: "baseline" | "cold-start" | "batch-size" | "ceiling" | "runtime";
...
function runLogicalScan(
  ...
  if (condition.kind === "runtime") {
    return runRuntimeLogicalScan(sccPath, fixtureDirectory, condition.batches.flat());
  }
...
function runRuntimeLogicalScan(
  sccPath: string,
  fixtureDirectory: string,
  paths: readonly string[]
): Omit<Sample, "condition" | "kind" | "round"> {
  ...
  const result = scanWithScc({
    cwd: fixtureDirectory,
    includePaths: paths,
    scanner: { executable: sccPath }
  });
  const wallMs = Number(process.hrtime.bigint() - startedAt) / 1_000_000;
  if (!result.ok) throw new Error(`final runtime scan failed: ${result.reason}: ${result.error}`);
  const normalized = result.measurements
    .map((measurement) => {
      const { payload, sourcePaths } = measurement;
      if (
        sourcePaths.length !== 1 ||
        sourcePaths[0] !== payload.path ||
        payload.codeLines !== 1 ||
        payload.decisionTokens.source !== "scc" ||
        payload.decisionTokens.value !== 0
      ) {
        throw new Error(`unexpected final runtime measurement: ${JSON.stringify(measurement)}`);
      }
      return `${payload.path}|${payload.codeLines}|${payload.decisionTokens.value}`;
    })
    .sort();
  const expected = expectedRuntimeDigestFor(paths);
  const measurementDigest = sha256(normalized.join("\n"));
  if (measurementDigest !== expected) {
    throw new Error(`final runtime measurement digest mismatch: ${measurementDigest}`);
  }
  const batchPlan = planSccBatches(sccPath, FIXED_ARGUMENTS, paths, 28_000);
  if (!batchPlan.ok) throw new Error(`final runtime batch plan failed: ${batchPlan.error}`);
  return { wallMs, invocationCount: batchPlan.batches.length, measurementDigest };
}
...
function expectedRuntimeDigestFor(paths: readonly string[]): string {
  return sha256(
    paths
      .map((path) => `${path}|1|0`)
      .sort()
...
function main(): void {
  ...
  try {
    ...
    const conditions: readonly Condition[] = [
      ...
      {
        id: "runtime-production-ceiling-28000",
        kind: "runtime" as const,
        batches: planByCeiling(sccPath, paths, 28_000)
      }
    ];
    ...
    for (const condition of conditions) {
      ...
      const conditionDigest =
        condition.kind === "runtime"
          ? expectedRuntimeDigestFor(conditionPaths)
          : expectedDigestFor(conditionPaths);
    ...
    const conditionSummary = conditions.map((condition) => {
      ...
      return {
        ...
        measurementDigest:
          condition.kind === "runtime"
            ? expectedRuntimeDigestFor(condition.batches.flat())
            : expectedDigestFor(condition.batches.flat())
      };
    });
    ...
    const summary: Summary = {
      ...
      protocol: {
        ...
        finalRuntimeCondition:
          "runtime-production-ceiling-28000 runs scanWithScc with the selected private ceiling and validates path/code/decision-token digest",
        planner:
