...
- [虚拟测量平台](../tooling/workspace.md#virtual-admission-workbench)：接入固定 profile 与经验向量；保持 legality、时间模型、随机源和策略可见信息分离。少量闭包 Check 只做真实集成，不能替代 Gate 单项时间校准。
- [算法对照 owner](../tooling/workspace.md#learned-admission-heuristic-对照记录)：使用冻结场景与参数比较，主指标 makespan、资源占用次之；规则有依据且无明显退化即可，不承诺真实加速。完整单项计时/竞争参数缺口不通过扩大算法搜索解决。
