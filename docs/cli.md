...
The supported integration boundary is a project-owned Run module. A project Run binds one Project
Definition and calls Package Run as `run(definition, controls)`. Other callers invoke that project Run
and never ask Product to discover configuration. `scripts/quality/scan.ts` is this repository's thin
dogfood caller.
