...
- `bun|scripts/validate.test.ts|root validate CLI runs every docs task by default`
- `bun|scripts/validate.test.ts|root validate CLI forwards focused docs selections`
  Proves:
...
- `bun|scripts/project-gate/index.test.ts|Project Gate entries, root binding, and controls > binds retained workspace verification names directly to the Gate profiles without disabled tags`
- `bun|scripts/project-gate/index.test.ts|Project Gate entries, root binding, and controls > keeps the explicit assurance identities and current profile membership closed`
- `bun|scripts/project-gate/index.test.ts|Project Gate entries, root binding, and controls > defaults to required and normalizes explicit profile plus repeatable disabled tags into opaque flags`
- `bun|scripts/quality/project-gate/project-definition.test.ts|Project Gate Definition > projects ordinary Check entries without a command catalog or policy`
...
- `bun|scripts/quality/project-gate/project-definition.test.ts|Project Gate Definition > keeps native Check outcomes transcript-free`
- `bun|scripts/quality/project-gate/project-definition.test.ts|Project Gate Definition > preserves two-step ast-grep process evidence and failures`
  Proves:
...
- `bun|scripts/quality/project-gate/process-check.test.ts|Project Gate process Check > avoids starting cancelled work and maps process/log boundaries to unavailable`
