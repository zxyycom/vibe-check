...
- `bun|src/package-checks/duplicate-detection/default-check.test.ts|default Check direct callbacks > detects project-relative duplicates through the public Check and fails the default strict aggregate`
...
- `bun|src/package-checks/duplicate-detection/jscpd/scanner.test.ts|quality jscpd wrapper failure projection > classifies commands missing after preparation as execution failures`
...
- Node Worker `error`, zero/non-zero `exit` without a reply, malformed reply, and synchronous `postMessage` failure each fail the complete analysis. Competing late message/error/exit callbacks after post failure or abort cannot create a second settlement; unsubscribe and terminate each run exactly once. Parent admission measures actual bytes, accepts exactly 8 MiB per file, rejects larger files or an aggregate over 64 MiB before Worker analysis, and maps an exact-path read failure to `source-unavailable`; none publish a metric prefix.
