...
- `bun|src/project-definition/project-definition.quiet-pass-policy.test.ts|Project Definition > normalizes executable quiet-pass policy and rejects other declarations`
- `bun|src/project-definition/project-definition.quiet-pass-policy.test.ts|Project Definition > ignores inherited quiet-pass policy while defaulting executable Checks`
...
- `enabledByFlags.when` and its recursive children accept nonempty string atoms or compatible raw AST. Direct `all` / `any` / `none` / `notAll` / `exactlyOne` / `not` builders preserve nesting, child order and multiplicity, while `changeFlag` retains its protected literal token. String, builder and raw forms normalize to the same frozen canonical condition and fingerprint; the legacy non-empty `{ flags, mode }` set is separately copied, de-duplicated and sorted. Mode or literal opt-in `propagateDependsOn: true` changes declarative identity, while omission preserves direct-selection compatibility.
- Empty or malformed flag sets, empty string atoms, unknown modes or control fields, non-literal propagation values, container declarations and the retired singular field fail Definition validation. Configured Git change sources require a safe nonempty comparison revision: option-like and U+0000-containing values fail Definition validation before author work.
...
- `bun|scripts/package/candidate/external-consumer/type-acceptance.test.ts|external consumer type fixture uses only its public strictness profile`
- `bun|scripts/package/candidate/external-consumer/type-acceptance.test.ts|external consumer type acceptance`
  Proves:

- The public package uses the `@zxyycom/vibe-check` import specifier and exposes only the documented generic authoring/run/Finding-presentation/waiver/cache/admission-simulation operations, direct flag-condition builders and `changeFlag`, the `defineAdmissionPolicy` inference helper, one composable default project-file selection, eight package-provided Check functions, eight named final-data parsers, and their required authoring/resolved/final/Record/reason/cache/admission-policy/admission-state type roots. Change-flag authoring additionally exposes only `CheckFlagConditionInput`, its normalized condition and shorthand, Git source/region/configuration, and file-centric evidence types; Definition-normalized controls and Git acquisition helpers remain internal.
...
- `bun|src/project-run/progress-rendering/invocation-progress-record-previews.test.ts|Package Run progress rendering outputs > omits a quiet pass before preview formatting without changing its final facts`
- `bun|src/project-run/progress-rendering/invocation-progress-record-previews.test.ts|Package Run progress rendering outputs > renders accepted quiet-pass Records while retaining complete Records and messages in final facts`
...
- Accepted `handoff: true` references are not read, summarized or presence-tagged by diagnostic logging. A same-Run dependency consumer can prove strict reference identity while both diagnostic and machine outputs omit its private bytes.
