...
Proves:
- Project Definition is a closed plain TypeScript value; its direct built-in/custom Check tree accepts ordinary built-in data and standalone field-aware adjustments while rejecting unknown patches, invalid metadata, accessors, and reflection failures without mutating caller input. It validates inherited scheduling metadata before work, canonicalizes equivalent declarative data for fingerprints, and keeps trusted bindings out of public data.
- Package Run validates one definition and closed controls before project functions, performs one Normalized-to-Resolved Check pre-work pass, prepares only selected built-ins, invokes TaskPlan factories only for applicable Checks, and projects direct/TaskPlan work through the shared Task engine without leaking functions into results.
- Cancellation is observed both before planning and at execution admission: no pending user work starts after abort, admitted work drains, and the retained snapshot distinguishes completed contained failure from cancelled unresolved Check facts.
