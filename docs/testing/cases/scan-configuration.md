...
- The public package uses the `@zxyycom/vibe-check` import specifier and exposes only the documented generic authoring/run/Finding-presentation/waiver/cache operations, the `defineAdmissionPolicy` inference helper, one composable default project-file selection, seven package-provided Check functions, seven named final-data parsers, and their required authoring/resolved/final/Record/reason/cache/admission-policy type roots.
- An ancestry-external TypeScript consumer imports and typechecks that public surface and declaration documentation without casts or manual dependency-read generics; it can declare `observes`, enumerate its direct settled outcome through the callback-local `dependencies.list()` surface, and still uses the producing parser for final data. It can author either a custom admission policy over the readonly graph/candidate/capacity/runtime DTO, which must synchronously return exact `select(taskId)` or `wait`, or a learned-critical-path policy with its local state directory; an async proposal, unknown policy field, and v1 `expectedDurationMs` field are rejected in installed declarations. The caller-keyed cache parser must likewise synchronously return a non-thenable typed value. The deeply frozen file-selection baseline composes into a consumer-owned selection, and every package Check export is callable and retains its typed parser relation.
