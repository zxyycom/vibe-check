...
- The public package uses the `@zxyycom/vibe-check` import specifier and exposes only the documented generic authoring/run/Finding-presentation/waiver/cache/admission-simulation operations, direct flag-condition builders and `changeFlag`, the `defineAdmissionPolicy` inference helper, the generic `commandCheck` constructor, one composable default project-file selection, eight package-provided fixed Check functions, eight named final-data parsers, and their required authoring/resolved/final/Record/reason/cache/admission-policy/admission-state type roots. `commandCheck` exposes only its six declared constructor/environment/output/final-data/unavailable-reason/Check types; private process runner types are not public roots. Change-flag authoring exposes one string-leaf `CheckFlagCondition`, `{ when, propagateDependsOn? }` enablement, Git source/region/configuration, and file-centric evidence types. The retired `CheckFlagConditionInput` and `CheckFlagEnablementMode` roots are absent, not internal; only Definition-normalized controls and Git acquisition helpers remain internal.
