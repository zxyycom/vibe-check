...
- `bun|scripts/project/gate/run.test.ts|Project Gate adapter closure > maps aggregate, definition warning, output and malformed facts to Gate exits`
  Proves:
- Required, complete `--all` and focused preset Gate selections produce the same eligible Check IDs that the bound Project Run passes to explicit `all` aggregation (not the `--all` selection), with unavailable propagation, included N/A failure and failed empty set.
