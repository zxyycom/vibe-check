...
- Named scripted clock phases distinguish valid zero spans from invalid clock samples; timing failure retains the fingerprint, admitted and accepted-wait counts, max-running, last-settled Task ID, queue peaks, and tail active count without fabricating time. Only when both diagnostics are disabled and the caller Hook list is empty does Scheduler add no measurement collector or clock reads; summary writer failures cannot revise the settled Scheduler result.
