...
- `bun|src/project-run/controls/flags.test.ts|Package Run flags > keeps matching roots direct unless their author opts into dependency propagation`
- `bun|src/project-run/controls/flags.test.ts|Package Run flags > starts each opt-in root dependency closure without selecting observations`
- `bun|src/project-run/controls/flags.test.ts|Package Run flags > expands a direct-selected dependency when an opt-in root requires its prerequisite`
- `bun|src/project-run/controls/flags.test.ts|Package Run flags > leaves pre-work cancellation ahead of flag dependency selection`
