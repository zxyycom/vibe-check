...
- `bun|src/project-run/project-changes-lifecycle.test.ts|Package Run project changes > rejects option-like Git comparisons before preparation or execution`
  Proves:
- Package Run validates closed definitions and controls before execution callbacks or outputs, rejecting unknown Run control keys. Its invocation control barrier checks cancellation and all declarative flag conditions before Scheduler admission; nonmatching Checks settle without author work. An admitted remaining Check runs optional preparation with detached frozen authored options and the invocation signal before its own callback; independent admitted Tasks can prepare concurrently, while block, throw, malformed messages/descriptors, and noncanonical prepared/fallback values settle only the owning Check unavailable without callback execution. Cancellation closes the execution phase without admitting new author work. Every ready executable Check receives only its public context, whose `project` value contains normalized `root` and canonical effective `flags`; trusted preparation/execution callbacks stay outside frozen facts, and Run derives no aggregate unless controls explicitly select one.
- The mapped `flags.test.ts` entities specifically prove pre-callback rejection for invalid flag input, caller rejection of the `vibe-check:change:` Product namespace, the canonical immutable `project.flags` callback snapshot, and declarative `enabledByFlags` predicates. The mapped Git/change lifecycle entities prove one data flow per invocation: a single frozen Git result supplies `project.changes` to both preparation and execution, and its derived protected tokens are unioned with caller tokens into one canonical effective flag set. That same frozen effective set drives selection and is the `project.flags` seen by both callbacks; `project.changes` remains the separate files/unavailable evidence channel. The first Git entity also covers `compareWith` as a branch, commit hash, `HEAD~N`, or tag revision. Committed, staged, unstaged, untracked, rename and deletion paths are deduplicated before frozen regions can emit their stable flag lists. For a nested project root, every Git delta is root-relative and a rename crossing that boundary retains only its in-root side, never a `../` path. A raw Git path containing a backslash is not converted to slash form before validation, so it cannot masquerade as an excluded nested path; that untrusted evidence produces the same unavailable fallback. A trustworthy zero-match result remains `{ ok: true, files: [] }` and leaves `project.flags` as caller flags only, while unavailable Git evidence has no fabricated files but conservatively injects every declared change flag into the same effective `project.flags` and selection input. Option-like comparison revisions are rejected as Definition configuration before either preparation or execution author work. The mapped `flag-dependency-selection.test.ts` entities specifically prove the separately owned dependency-selection behavior: matching opt-in roots activate their transitive `dependsOn` closure; activated dependencies may run despite their own unmet predicate; directly selected intermediates still expand when required; repeated roots deduplicate shared prerequisites; `observes` remains outside the closure; and pre-work cancellation remains earlier. Matching conditions otherwise execute the ordinary task-local preparation/callback path; nonmatching conditions settle before author work and enter the Scheduler as pre-admission non-passed Task results. This private selection appears in neither a member-list callback capability nor stable RunResult, machine or diagnostic fields.
