...
- `bun|src/package-checks/markdown-link-validation/default-check.test.ts|default Check direct callbacks > cancels after parse-facts publication without reporting stale findings`
  Proves:
- A completed traversal publishes only the documented safe local-reference Record projection and parser-validated exact final counts; root-external findings retain no destination material. The omitted finding policy defaults to non-blocking and retains the Records/final data in a passed outcome with an actionable warning, while explicit blocking retains the same evidence and settles failed with an actionable error. Enabled parse-facts cache hit, miss, hostile payload, storage failure, and exact-byte source/target invalidation preserve the same Check settlement and do not create cache output facts. Cancellation before source work or after an immutable parse-facts entry publishes remains unavailable with no link-finding Record; the latter entry is performance state rather than settlement replay. Completed normal/rejected findings additionally project at most ten Check-owned summaries plus an exact omitted count; those summaries retain source navigation and closed reason while never copying the unsafe destination.
