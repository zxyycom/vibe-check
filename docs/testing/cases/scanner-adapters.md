...
Entities:
- `bun|src/product/quality-core/src/check-record/builtins/file-metrics.test.ts|file-metrics built-in Check > fails current unavailable process CSV and out-of-scope batches without records`
- `bun|src/product/quality-core/src/check-record/builtins/file-metrics.test.ts|file-metrics built-in Check > keeps file record identity stable when only current location changes`
- `bun|src/product/quality-core/src/check-record/builtins/file-metrics.test.ts|file-metrics built-in Check > retains an earlier valid record when a later out-of-scope batch is rejected`
...
- `bun|src/product/quality-core/src/check-record/builtins/file-metrics.test.ts|file-metrics built-in Check > uses the frozen changed-file scope instead of baseline delta to classify changed records`
Proves:
...
Owner: `docs/quality-metrics.md#built-in-checks-与-exact-inputs`
Entities:
- `bun|src/product/quality-core/src/check-record/builtins/function-metrics.test.ts|function-metrics built-in Check > distinguishes successful zero-function work from no input`
- `bun|src/product/quality-core/src/check-record/builtins/function-metrics.test.ts|function-metrics built-in Check > fails unavailable execution invalid and out-of-scope current batches without records`
- `bun|src/product/quality-core/src/check-record/builtins/function-metrics.test.ts|function-metrics built-in Check > keeps complete current records when reference scope is incomplete`
...
- `bun|src/product/quality-core/src/check-record/builtins/function-metrics.test.ts|function-metrics built-in Check > retains ambiguous function instances as changed without inventing regressions`
Proves:
...
Owner: `docs/quality-metrics.md#built-in-checks-与-exact-inputs`
Entities:
- `bun|src/product/quality-core/src/check-record/builtins/duplicate-detection.test.ts|duplicate-detection built-in Check > distinguishes zero findings and no input and fails unavailable invalid and out-of-scope batches`
- `bun|src/product/quality-core/src/check-record/builtins/duplicate-detection.test.ts|duplicate-detection built-in Check > keeps equal-shape fragments distinct and their identities stable across line movement`
...
- `bun|src/product/quality-core/src/check-record/builtins/duplicate-detection.test.ts|duplicate-detection built-in Check > retains an earlier record when a later area batch fails and keeps reference failure separate`
- `bun|src/product/quality-core/src/check-record/builtins/duplicate-detection.test.ts|duplicate-detection built-in Check > reuses cache revalidates cached paths and keys backend arguments privately`
Proves:
