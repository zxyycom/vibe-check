...
| 修改 package README template、可验证 API 示例、projection registry/renderer/CLI 或 generated README/JSDoc example                    | [脚本工具](script-tooling.md)、[编码规范](coding-style.md)                                       | 先区分可编辑输入（template、examples、registry、source JSDoc）与 generated output（root README、managed JSDoc example）；再读 [Configuration](configuration.md)、[Output](output.md)、`docs/examples/package-api/`、declaration owners 与 candidate tests |
