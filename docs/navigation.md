...
| 修改 machine DTO/schema、contract-valid set / published set、serialization、validator、publication/evidence、console/report、artifact 或 output status | [Output](output.md)、[编码规范](coding-style.md)                                                 | [Configuration](configuration.md)、[Quality Metrics](quality-metrics.md)、`docs/schemas/`、`docs/examples/` 与 output tests |
| 修改 package Markdown template、Check guide、可验证 API 示例、projection registry/renderer/command 或 generated Markdown/JSDoc example | [脚本工具](script-tooling.md)、[编码规范](coding-style.md)                                       | 先区分可编辑输入（templates、Check guides、examples、registry、source JSDoc）与 generated output（root README、API mechanics guide、managed JSDoc example）；再读 [Configuration](configuration.md)、[Output](output.md)、`docs/examples/package-api/`、declaration owners 与 candidate tests |
| 修改开发脚本、Project Gate、共享 process/repository capability、根级诊断/值守卫、docs validator 或 quality wrapper                               | [脚本工具](script-tooling.md)、[编码规范](coding-style.md)                                       | 对应 `scripts/**`；docs provider 见 `scripts/docs/**`，docs acceptance 见 `scripts/validation/documentation/**`；涉及产品时再读正式产品入口及 consumer contract |
