...
| 修改开发脚本、Project Gate、共享 process/repository capability、根级诊断/值守卫或 docs validator                                               | [脚本工具](script-tooling.md)、[编码规范](coding-style.md)                                       | 对应 `scripts/**`；docs provider 见 `scripts/docs/**`，docs acceptance 见 `scripts/validation/documentation/**`；涉及产品时再读正式产品入口及 consumer contract |
