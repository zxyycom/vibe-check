...
只影响后续样本；observer failure 由 helper 包含。参数、安全与使用方法由[调度指南](../guides/scheduling.md#learned-critical-path-strategy)拥有。
