...
启用 Run progress 后，renderer 另行最多预览五条 messages 和五条 Records；这是展示限制，
不会改写 terminal result 或完整 facts，详见 [Check messages 与受管 progress](../api-mechanics.md#check-messages-与受管-progress)。
