...
Explore 的输出是清楚的问题定义、事实依据、方案取舍、已确认决策、开放问题和阶段建议。可以阅读文件、搜索代码、调查架构、检查 OpenSpec 上下文，并按授权沉淀探索材料；应用代码实现由对应实施流程承接。
...
5. 用户希望把探索结论写入关联 change，或判断当前方向是否已经可以进入实施准备。
...
9. 沉淀探索结果和修改已有 change 时，保存支撑当前结论的目标、范围与非目标、关键边界、依赖、风险、证据、开放问题、启动条件和高层验收方向；设计、任务和验收依据在进入实施准备后由 `$openspec-propose` 承接。
...
5. 未回答问题保留在 `## Open Questions`；apply 的进入条件是全部问题已有答案，且已收敛条目含义明确。
...
5. 决策记录：只约束一个 change 时更新对应 artifact；跨 change 仍有长期影响时按 `$decision-records` 维护。
6. 下一步建议：提出继续探索、创建 change、补充 artifact、做 spike 或进入实现流程的具体选择。

## 边界

1. Explore 负责澄清问题、调查事实、比较方案和沉淀决策；应用代码实现进入对应实现流程。
2. 只读调查命令可直接执行；写入范围来自用户要求的 OpenSpec artifacts 或本次任务明确指定的文档。跨 change 长期决策写入还需满足 `$decision-records` 的确认和维护契约。
