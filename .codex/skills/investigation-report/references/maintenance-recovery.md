...
| 手工正式来源变化，或索引缺失、损坏、陈旧 | 核对完整正式报告与资源后运行 `sync-index`（默认写入，`--preflight` 零写入预演）；publish 只处理所选候选，不接纳这些变化。 |
| 严格 check、关系事务、正式删除或 stage 报索引陈旧或无效 | 未解释的异常先运行默认全量 `check` 区分来源与索引问题；已知合法来源变化直接 `sync-index` 发布当前投影，再重试原动作。 |
...
对账以完整旧状态或新状态为目标。publish 越过提交点后，正式报告与索引已建立；discard 越过提交点后，被删除的候选或正式报告已退出自己的集合。此时清理残留不是重做领域操作的理由。

stage 的 `index` scope 只拥有目标索引的 pending 路径；`domain`/`all` scope 只拥有所选报告及其 owner 资源树（及 `all` 的索引投影）。其他命令按契约维护各自工作区范围。rename 同时涉及来源身份、关系、位置与资源 owner，必须整体核对，不能仅凭单个文件已改名判断成功。
