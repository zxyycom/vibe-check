...
- [ ] 1.2 对 runtime schema 无法完整表达的语义增加 path-aware post-validation：time zone、`minimumTokensByCodeArea` key 与 `codeAreas` reference 及其它 task 0.3 已确认关系；raw input 保持 `unknown`，成功后返回 detached value。本解耦保持 current finite-number acceptance，不自行增加 positive/integer constraints。
...
- [ ] 2.1 定义 readonly `ScannerDependencySnapshot` 与 file/function/duplication capability-specific slices；集中 built-in executable/args、host-platform resolution、availability protocol inputs 与 bounded concurrency，不把 duplicate per-path format detection 提升进 generic snapshot，也不建立 provider/plugin hierarchy。
- [ ] 2.2 把当前 `VIBE_CHECK_LIZARD_CMD`、`VIBE_CHECK_SCC_CMD` / `_ARGS`、`VIBE_CHECK_JSCPD_CMD` / `_ARGS` 作为 internal operational compatibility 一次读取并严格校验；non-empty `*_CMD` string 替换 executable，unset/empty 表示无 override，executable existence 留给 eligible availability；non-empty `_ARGS` 必须是 JSON string array，unset/empty 产生空 additional-args list。全部 supplied non-empty inputs 在 eligibility 前完成 shape validation，invalid shape 使用 typed operational error / exit `2`、隐藏完整 value 且不从 project config fallback。Lizard args 固定 `-m lizard`，不新增未存在的 args override。
- [ ] 2.3 先写/复用 failing characterization，再让 duplicate adapter 对 exact inputs 省略 jscpd format override，并把 bounded concurrency 迁入 dependency slice；覆盖 pure `.ts` / `.rs`、mixed-area per-extension behavior、minimum-token default/override、scope exclusion、insufficient-area no-work 与 cache identity。不能恢复 public format field 或建立无消费者的 format abstraction。
...
- [ ] 2.5 修改 baseline orchestration，使 current 与 baseline 复用 invocation dependency snapshot 但独立计算 revision eligibility；证明 current no-input / baseline eligible、skipped/no-input 不检查 dependency availability 或启动 process（caller-supplied operational input 仍按 2.2 校验），以及 environment 不被第二次读取。
...
- [ ] 4.3 扩展 formal-entry failure coverage：invalid semantic document / legacy shape 在 exit `3`、invalid operational override 在 exit `2`、eligible dependency unavailable 进入 failed capability，以及 no-input/skip 不检查 dependency availability 或启动 process；所有 caller-supplied operational inputs 仍在 eligibility 前校验，所有 pre-scan boundary error 均无 banner/scanner/cache/artifact side effect。
...
- [ ] 5.2 更新 Architecture、Scanner Dependencies、Scan Scope、Quality Metrics、CLI、Testing 与 navigation owners，并同步 `duplicate-scanning` main spec；分别记录 dependency snapshot/data flow、duplicate exact-input/format-detection boundary、scope reuse、semantic checks/acceptance、formal-entry fixture path 与验证边界；每项规则只在对应 owner 完整表达。
- [ ] 5.3 写入 approved old-to-new migration table：threshold sections、accepted-warning identity、removed dependency execution settings、supported operational override 落点、无 Lizard args override、removed duplicate format filter 和 internal concurrency；说明 binary/config pair rollback，并只提供一个 canonical semantic example，不复制完整 legacy config。
...
- [ ] 6.4 运行 schema generation drift、independent schema compile/canonical validation，并定向搜索 public runtime schema、config examples、fixture、Configuration docs 和 external starter contract，证明无 `lizard` / `scc` / `jscpd` / `command` / `args` / dependency concurrency / backend format filter；搜索须排除明确允许的 internal adapter、machine output 和 historical migration 说明。
...
- [ ] 6.7 运行本 change 及两个 dependent active changes 的 OpenSpec strict validation、`bun run decisions:check` 和 `git diff --check`；确认只在任务批准范围修改文件，0.x gate state、implementation evidence 和剩余风险可独立恢复。
