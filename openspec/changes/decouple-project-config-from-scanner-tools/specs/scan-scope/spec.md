...
#### Scenario: Git collection uses selected semantic include paths
...
#### Scenario: Explicit semantic config changes candidate eligibility
...
#### Scenario: Git failure keeps the same semantic config
...
#### Scenario: Selected config does not inherit hidden exclusions
