...
- [x] 1.1 用正式 Product CLI 与现有 fixtures 建立 baseline，覆盖 complete-passed、
...
- [x] 1.2 把 baseline 固定为 projection regression fixtures/tests，而不是 canonical examples
...
- [x] 2.1 先增加 projection tests：`MachineMetricsV1` 保留经 1.1 确认的 field set，并设置
  ...
  产生。
- [x] 2.2 先增加 schema contract tests，固定 JSON Schema 2020-12、canonical paths、`$id`
...
- [x] 2.3 在 `src/product/**` 的 Output owner 内实现 runtime schema-authoring source、由该
...
- [x] 2.4 实现 deterministic serializers：metrics 使用 two-space JSON 且无 final LF；
  warnings 每条使用 compact JSON + LF，empty channel 产生 zero bytes。所有 stream records
  必须来自 machine DTO，而不是第二条 core projection path。
- [x] 2.5 从 runtime source deterministic 生成并 checked in
...
- [x] 3.1 先增加 positive byte-grammar tests。Metrics 覆盖 strict UTF-8、BOM rejection、
...
- [x] 3.2 实现 product-owned warning-stream validator：以 bytes 为输入，成功时返回完整 typed
  ...
  partial records。
- [x] 3.3 先为 artifact-set invariants 增加独立 success/failure proofs：stream/channel deep
...
- [x] 3.4 实现 artifact-set validator，组合 metrics/warning schemas、两个 stream validators
  ...
  logic。
- [x] 3.5 先增加 publication failure tests，覆盖 candidate validation、prior-file cleanup、
...
- [x] 3.6 将 finish flow 实现为 `core validate → one DTO → three in-memory candidates →
...
- [x] 3.7 增加 formal-entry outcome tests，证明 complete-passed、complete-warning、
...
- [x] 4.1 建立 fixed fixture generator，按 specs 的五行 outcome matrix 从 production
...
- [x] 4.2 生成并 checked in 五组 examples；验证 repeated generation byte-stable、zero-warning
...
- [x] 4.3 扩展 independent docs validator：显式注册并 strict compile 两个 current schemas，
  独立检查 example UTF-8/framing/schema 与全部 set invariants；不得 import product validator
  作为 acceptance implementation。
- [x] 4.4 增加 runtime/docs focused mutations 与 generation drift proofs，覆盖 specs 列出的
...
- [x] 5.1 先增加 `quality:annotate` CLI tests：固定 `[warnings-path] [limit]`、default path、
...
- [x] 5.2 让 `quality:annotate` 通过 shallow product export 复用 warning-stream validator，
  以 bytes 读取 input；完整 validation 后才按既有规则过滤 `info`、应用 limit、映射 schema
  声明的 render fields。Quality annotations 继续 non-blocking。
- [x] 5.3 增加正式 producer-to-consumer acceptance：Product CLI 生成 non-empty/zero-byte
...
- [x] 5.4 将 acceptance 接入 required workspace profile；workspace verifier 只调度 child、
  保留 actionable output 并传播 result，dogfood wrappers 与 package `quality:*` 保持 Product
  CLI pass-through。

## 6. 同步 owners 并验证交付

- [x] 6.1 更新 Architecture、Output、Quality Metrics、CLI、Testing、Script Tooling、
...
- [x] 6.2 先运行最窄的 DTO/schema/mapper/serializer/output/annotation/docs tests；测试变化前后
  运行 `bun run test-evidence:check`，并确认受影响 Cases 的 Owner/Entities/Proves 闭合。
- [x] 6.3 运行 `bun run typecheck:product`、`bun run typecheck:scripts`、
  `bun run lint:product` 与 `bun run lint:scripts`。
- [x] 6.4 运行 schema/example regeneration drift check、targeted producer-to-consumer
  acceptance、`bun run validate` 与 `bun run verify:vibe-check-workspace:required`。
- [x] 6.5 重放 `quality:check`、`quality:full-check` 与 `quality:gate`，确认 current artifacts、
