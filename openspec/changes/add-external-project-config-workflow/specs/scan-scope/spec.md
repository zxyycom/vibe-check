...
读取 built-in dogfood values、发现 config files、重新应用 environment overrides，或按
selection source 改变 include/exclude semantics。

#### Scenario: Explicit and discovered configs share scope behavior

- **WHEN** 相同 complete config 内容分别通过 explicit path 与 root discovery 选择
