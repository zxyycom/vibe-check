...
最小Record contract已由[`establish-minimal-check-record-contract`](../archive/establish-minimal-check-record-contract/)完成。本Change继续等待[`add-typed-check-dependency-outputs`](../add-typed-check-dependency-outputs/)、[`add-check-terminal-messages-and-visibility`](../add-check-terminal-messages-and-visibility/)和[`ship-public-package-api-documentation`](../ship-public-package-api-documentation/)收敛其余首次公开package inputs。随后重新准备或安全复用matching candidate、校验installed entry，并从正式root entry运行focused/native/process/candidate tests、required/full acceptance和partial eligibility smoke。
