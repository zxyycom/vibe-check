...
当前 <code>RunControls</code> 没有 project-defined control flag，<code>CheckExecutionContext.project</code> 也不能读取它。项目若希望某个 Check 因本次运行的简单控制标志而不适用，只能在 Product 外重建控制路径，或错误地把一次调用的开关放进 Definition-owned static <code>options</code>。
...
完成后，<code>run(definition, { flags })</code> 接受 optional string flag collection，并向每个 Check callback 提供始终存在的 <code>context.project.flags</code>。Product 在任何 callback、scanner、cache 或 reporter work 前验证该 collection，去重、规范排序并冻结 snapshot。
...
1. <code>flags</code> 是 optional 的 readonly string input；callback 侧总能读取一个 frozen、去重、排序后的 readonly string collection，省略时为空 collection。
2. 输入只接受 non-empty string；非 array、非 string 或空 string 在任何 Check callback 前按既有 <code>invalid-run-controls</code> configuration failure 拒绝。
...
- Control validation 与一次性 context construction：<code>src/product/run/control-validation.ts</code>、<code>src/product/run/project-context.ts</code>；直接行为证据从 <code>src/product/run/index.test.ts</code> 开始。
