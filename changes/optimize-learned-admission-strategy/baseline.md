...
| Workload | current live policy, required/all Project Gate selections |
| Warm-up | each selection warmed once; warm-ups excluded |
| Accepted observations | five sequential observations per selection |
...
| Selection | Raw initial observations | p50 | p95 | CV |
| --- | --- | ---: | ---: | ---: |
| required | 9552.1, 9198.4, 9586.1, 9604.9, 8894.1 | 9552.1 | 9604.9 | 2.93% |
| all | 16086.6, 15367.4, 15500.3, 15618.3, 15479.4 | 15500.3 | 16086.6 | 1.62% |

These samples are live-environment attribution observations only, not the primary timing baseline. Future A/B uses formal-command outer wall as pre-registered primary, one warm-up per variant/selection, and preserved raw evidence. Its fixed sequence is: odd groups baseline-required→candidate-required→baseline-all→candidate-all; even groups candidate-required→baseline-required→candidate-all→baseline-all. It must not compare a later result directly to these values as a candidate gain.
...
| Selection | Span p50 / p95 | Util p50 / p95 | Accepted wait p50 / p95 | Tail p50 / p95 |
| --- | ---: | ---: | ---: | ---: |
| required | 9029.513 / 9061.199 | 0.996207 / 0.996773 | 8909.893 / 8930.252 | 17.512 / 18.317 |
| all | 14916.617 / 15483.082 | 0.997688 / 0.998002 | 14774.031 / 15346.202 | 19.783 / 21.176 |
