...
- [x] 0.1 已确认三个 artifacts 共享同一目标：把前置 Product 运行内核投影为 API-only Bun npm package；package 公开配置定义函数、Package Run、三个 non-callable built-in values 和必要 types。
...
- [x] 0.14 已把多个 Plans 收敛为单向 owner 顺序：三个前置 Changes 交付 definition/run、descriptor/tree/options、Check-scoped cap 与 Product 运行内核；本 Change 只消费并添加 package/release projection。
- [ ] 0.15 核对三个前置 Changes 已完成并归档，current-contract fields、Project Definition validator、Product 运行内核、descriptor/tree/options、Check-scoped cap/admission-drain、Task/dependency semantics、operational snapshot、JSON hard cut 和两文件 usage 均有 owner docs 与目标 tests。
...
- [ ] 1.1 核对并扩展 `src/product/**` current public-contract source：保留前置 Changes 的 definition-facing names、三个 built-in descriptor values 及 `.replace()` / `.append()` member names、Check tree/cap types、defaults、environment/dependency identifiers；按 license decision、release history 与 evidence 添加 package name、MIT/license、candidate version、support matrix、system prerequisites、manifest/release fields 和 consumer map；不写 placeholder 或项目文件路径。
...
- [ ] 1.3 从 current public-contract source 和 Product 运行内核建立 public entry：导出配置定义函数、Package Run、`duplicateDetection`/`fileMetrics`/`functionMetrics` non-callable values（含 current-contract owned `.replace()` / `.append()` member names）与必要 definition/tree/cap/control/result types；runtime callable export inventory 恰好为两个 functions，不泄漏 Core、manager、scheduler、Task、bindings 或 convenience operation。
...
- [ ] 1.11 建立安全临时 Bun acceptance：只安装 exact tarball、declared dependencies 与明确 prerequisites；创建项目配置文件，直接导入三个 built-in values并验证 descriptor `.replace()` / `.append()`、mixed tree、group/leaf cap override、active-cap min/reservation-drain；创建项目运行脚本，再由 separate caller 只调用项目 Run；验证 project functions、representative `TaskPlan` dependency/parallelism/resources、default effects、complete result、failures、cancellation 和 concurrent invocation。
...
- [ ] 2.1 对 current public-contract source 执行 owner-to-artifact comparison，证明 manifest、两个 callable exports、三个 non-callable built-in values、definition/tree/cap types、defaults、environment/dependency identifiers、docs 与 fixtures 只使用 owned values；项目文件名未成为 package contract。
- [ ] 2.2 运行配置定义 authoring、descriptor/tree/options、Check-scoped cap、definition/control/result、expected failures、Task/Check/Record projection、cancellation、concurrent invocation、operational resolution 与 effects 的最窄 tests；证明 expected failures 不依赖 console、exit code 或 exception text。
...
- [ ] 2.7 在 isolated installed project 重跑 separate-caller acceptance：确认 descriptor values/options、public tree/cap types、validators、Task/Check/Record results、built-in identities 与 effects 来自同一 version；runtime callable exports 恰好是配置定义函数和 Package Run，internal paths 不是 supported imports。
