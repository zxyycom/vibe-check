...
`adopt-typescript-project-definition` 已单独拥有 Project Definition 的 authoring、selection、private loading、normalization、JSON hard cut 与 foundation handoff。本 Change 先冻结它需要消费的公共契约，待该 Change 提供可用 runtime seam 后，再完成 public entry、package projection、Product CLI hard cut 与 exact-tarball acceptance；两个 Change 不平行实现同一 loader 或配置迁移。

Vibe Check 继续作为产品显示名，公开 registry package 使用 unscoped `vibe-check`，以 MIT 许可正常发布。Public exports/symbols、固定 Project Definition 路径和运行标识是 package 工程需要在 candidate 前闭合的契约值，但不要求产品 owner 逐项命名，也不能从 root manifest、源码、Change 名称或示例偶然继承。
...
首个 package 的产品边界已经确定：unscoped `vibe-check`、public npm distribution、MIT、`0.0.x` prestable、Bun-only direct import、API-only execution，以及允许由配置显式提供 Python、`scc` 等外部程序位置。Check 的配置字段和产品语义保持 scanner-tool 中立；具体工具并不隐藏，运行依赖绑定也不改变 Check 的身份或含义。工程实施在 candidate 前选择 root export、runtime/type symbols、固定路径、effect 路径和 operational identifiers，并把当前值写入一个 package-private current public-contract source；manifest、public entry、declarations、docs、fixtures 与 acceptance 从该 source 生成或做单向一致性核对。
...
- 消费 `adopt-typescript-project-definition` 提供的 definition/runtime seam，建立 Product-owned Bun package host、operational precedence、默认工具 effects 和 public result projection；
...
- 在 publishable candidate 形成前由工程闭合 imports/exports、symbols、固定路径、默认 effect 路径和 environment identifiers，并从唯一 current public-contract source 派生或核对全部消费者。

非目标：执行 `npm publish`、管理 registry credentials 或 Trusted Publishing；重新命名 Vibe Check；承诺 Node.js 或 dual-runtime direct import；在本 Change 中重新实现 Project Definition loader、JSON migration 或 foundation handoff；公开 worker/process module、IPC protocol、Core、manager、scanner adapter 或 scheduler；提供 public `init`、bootstrap、template/resource API 或第二套 Product CLI contract。
...
- Public imports/exports、两个操作和必要类型的 symbols、固定 Project Definition path、默认 output/cache path 及 supported environment identifiers 由工程在 candidate 前闭合，并由 current public-contract source 唯一拥有。
