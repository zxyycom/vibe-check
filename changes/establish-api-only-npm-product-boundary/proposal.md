...
本 Change 在 `adopt-typescript-project-definition` 完成并归档后，把其 Product 运行内核投影为 API-only npm package。首个 package 只支持 Bun，runtime callable exports 恰好是：
...
前置 Change 已负责 Project Definition authoring/validation、Package Run 的 Product 运行内核、项目函数调用、Task 调度、operational dependency snapshot、JSON hard cut 和两文件使用模式。本 Change 不重新设计这些语义，只负责：
...
- public runtime callable exports 只有配置定义函数和 Package Run，另加必要 public types；
...
- 验证并消费前置 Change 交付的 Product 运行内核、current-contract fields 和 canonical usage；
...
- 建立两个 public callable exports、必要 types、Bun default host 和 structured result projection；
...
- 前置 Change 已完成并归档；本 Change 只消费其 owner docs、current-contract fields、Product 运行内核和目标测试，不建立竞争 owner。
- AI 或工程实现者能从 Design 区分配置定义函数、Package Run 和项目 Run，并恢复完整调用方向。
- Public runtime callable export inventory 恰好包含配置定义函数和 Package Run；必要 types 不形成额外 operation。
