...
- [ ] 1.4 实现一次反向graph score计算和learned-critical-path admission policy，使其在relation/mutex eligible candidates与capacity facts上作select/wait；Scheduler保持selected-capacity、reservation-set target candidate与wait-drain guard，不重演policy reservation/fairness策略；static priority只作同分tie-break。
