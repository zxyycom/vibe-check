...
现有 `RunResult.checkDurations` 已如实报告本次实际执行 Check 的 monotonic active duration，但 Product 不跨 invocation 保存或消费这些事实。要求每个 Check 手工填写预计耗时会让能力在大 Definition 中难以采用；把可变统计塞入 `cacheJsonByKey` 又会违反该 helper 的 caller-keyed immutable result 契约。因此需要一个 Scheduler-owned、显式授权、失败时只影响优化的有界历史 owner。

## Outcome

项目在 Definition 的 Scheduler 设置中选择 `learned-critical-path` 并提供一个 cross-run state directory 后，所有实际启动的 Check 自动贡献真实 active-duration 样本。下一次 Run 在任何 Task admission 前加载有界历史，形成 immutable per-Task duration prediction 与 estimated downstream critical-path score；admission policy 只在 Scheduler 已允许比较的候选中优先选择更高分 Task。
...
省略 learned setting 时继续使用当前 static-priority policy，不读取或写入历史。学习历史和预测只改变 admission timing，不改变依赖、观测、mutex、capacity、reservation、Check outcome、Record、aggregation、machine publication 或结果排序。

## Scope

### Intended Change

- 扩展由 `expose-custom-admission-selection-policy` 建立的 closed Definition admission-policy union：省略时是当前 `static-priority`；`learned-critical-path` 需要显式 `stateDirectory`。相对目录从 effective project root 解析，绝对目录直接使用；不存在隐藏的用户级或仓库级默认写入。
...
- 在 final directed readiness graph 上计算 `estimatedDurationMs + longest estimated downstream path`。选择继续服从 sticky reservation 与当前 tightening/constrained/ordinary 层级；同一层级和既有 cap tie-break 内先按 critical-path score 降序，再按 `admissionPriority` 降序，最后按现有 canonical tie-break。
...
- `require-passed-dependencies-and-observe-outcomes` 必须先闭合 success dependency和outcome observation的 directed readiness edge；关键路径同时读取两者，但不把 outcome predicate变成预测输入。
...
- 未声明或显式选择static policy时，不创建state directory、不读写history，admission trace、terminal facts和所有public outputs保持当前static-priority兼容行为；两种static authoring形状得到相同的新canonical fingerprint。若schema演进使它与变更前fingerprint不同，实施证据必须明确记录并重建匹配baseline，不能伪称旧fingerprint未变。
...
- sticky reservation不能被高score或高priority Task偷取；blocked、mutex/capacity-ineligible或未ready Task不能被policy选择。static priority在learned mode只处理同分，不能让人工排序普遍屏蔽预测。
