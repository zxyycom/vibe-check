...
本文件只拥有本产品路径的阶段顺序与 public-readiness 输入关系：三个上游 Change 与首轮 Gate build 已归档，已归档的 [repository hard cutover Change](archive/replace-workspace-verifier-with-project-gate/) 已交付 [gate-handoff.md](archive/replace-workspace-verifier-with-project-gate/gate-handoff.md)。下一步先实施最小 Check Record hard cut；typed Check dependency outputs 与首版 result presentation 都依赖该事实，但彼此不互相阻塞。两者完成后再冻结 package API documentation 与 native Check authoring，刷新 Gate optimization evidence，最后才进入 publish。log evidence boundaries 是不阻塞本路径的相邻 Draft。
...
[archived] establish-npm-package-candidate-and-quality-dogfood ┘                                                    │
                                                                                  ...
                                                                                  establish-minimal-check-record-contract
                                                                                           │                    │
                                                                                           v                    v
                                                                         add-typed-check-dependency-outputs   add-check-associated-result-presentation
                                                                                           │                    │
                                                                                           └─────────┬──────────┘
                                                                                                     v
...
| [establish-minimal-check-record-contract](establish-minimal-check-record-contract/) | 用 `records.report({ id }, data)` 建立任意 custom/default Check 共用的最小 Record contract，并硬切 Core `{ checkId, id, data }`、旧 Record 直接消费者与 machine v4。 | Public declaration、generic readback/Check-owned parser boundary、runtime/Core、comparison/reference removal、policy/Gate、machine v4 与 isolated consumer evidence；typed dependency 和 presentation 据此使用同一事实结构。 | Cross-Check dependency reader/inference、public presentation grammar、非 comparison execution-input migration、Gate 切换、registry publish。 |
...
| [add-check-associated-result-presentation](add-check-associated-result-presentation/) | 交付 terminal Record presentation 与显式 Check visibility。 | Minimal Record 完成后的 implementation、declaration、output 与 isolated-consumer evidence。 | Typed dependency reader、Gate transcript、durable log protocol；typed dependency 不是其前置。 |
