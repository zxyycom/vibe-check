...
本文件只拥有本产品路径的阶段顺序与 public-readiness 输入关系：三个上游 Change、首轮 Gate build、repository hard cutover 与最小 Check/Record hard cut 已归档；[repository hard cutover Change](archive/replace-workspace-verifier-with-project-gate/) 已交付 [gate-handoff.md](archive/replace-workspace-verifier-with-project-gate/gate-handoff.md)。当前 typed Check dependency outputs 与 terminal messages/visibility 以已落地的最小 facts 为共同输入，但彼此不互相阻塞。两者完成后再冻结 package API documentation 与 native Check authoring，刷新 Gate optimization evidence，最后才进入 publish。log evidence boundaries 是不阻塞本路径的相邻 Draft。
...
| [add-check-terminal-messages-and-visibility](add-check-terminal-messages-and-visibility/) | 交付 `CheckResult.messages`、`RunResult.checkMessages` 与 `always | attention` visibility 两个主要能力。 | Closed terminal validation、Core acceptance handoff、canonical RunResult ordering、Definition fingerprint、TTY/plain matrix、Project Gate `command-failed` message 与 isolated-consumer evidence。 | Live/intermediate output、Core/Record/machine messages、typed dependency reader或 durable log protocol。 |
...
[`add-check-terminal-messages-and-visibility`](add-check-terminal-messages-and-visibility/) 是首次公开 package 的硬前置：它让需要补充提示的 Check 通过 `CheckResult.messages` 按需附带 level/code/message，settlement 时呈现并由 `RunResult.checkMessages` 返回，同时交付 `always | attention` visibility。它明确不提供 live/intermediate output、不把 messages 写入 Core/Record/machine；Project Gate process Check 是当前安全摘要实例，不是 consumer 迁移。

Supporting Check 的 structured facts 与 lifecycle events 当前始终产生。Typed dependency output Change 拥有依赖读取，terminal messages/visibility Change 拥有 human visibility 设计以及 messages 与 owning Check 的组合归属；其 Plan 已冻结完整 display matrix和fingerprint边界。
