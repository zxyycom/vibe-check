...
| [add-typed-check-dependency-outputs](add-typed-check-dependency-outputs/) | 保留string direct edge，以`dependencies.get(checkId)`读取canonical final data，再由producer `parseData`恢复typed data。 | Plan artifacts已记录provider-local Data prototype；实施需补runtime direct authorization、changed-files多consumer、四态settlement、declaration emit与machine/external readback evidence。 | Compile-time dependency-ID relation、supplemental Record getter、parser registry、search/query、第三Core entity、其它execution-input cleanup、presentation visibility。 |
