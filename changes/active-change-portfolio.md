...
1. **Project Gate 与 package 交付：** 三个上游能力 Change、首轮 Gate build、repository hard cutover 与最小 Check/Record hard cut 均已归档；[repository hard cutover Change](archive/replace-workspace-verifier-with-project-gate/) 交付了 [gate-handoff.md](archive/replace-workspace-verifier-with-project-gate/gate-handoff.md)。[Configuration](../docs/configuration.md) 与 [Output](../docs/output.md) 已记录终态 messages 与显式 visibility 两项当前 Product 能力。`add-typed-check-dependency-outputs` 是仍未完成的同级 Change，**不是** `add-check-terminal-messages-and-visibility` 的前置；`add-typed-check-dependency-outputs` 与已交付的 `add-check-terminal-messages-and-visibility` 是 `ship-public-package-api-documentation` 和 `align-project-gate-with-native-check-authoring` 的并列输入。完整依赖与 handoff 由 [Vibe Check package 与 Project Gate 交付导航](vibe-check-package-and-gate-delivery.md) 唯一承接。
...
| [add-check-terminal-messages-and-visibility](add-check-terminal-messages-and-visibility/) | 当前 Product 同时提供两项主要能力：Check 在 `CheckResult.messages` 附带 level/code/message，并由 `RunResult.checkMessages` 返回；Check 以 `always | attention` visibility 控制终态人读行。 | [Check 终态消息与显式 visibility](../docs/decisions/allow-check-terminal-messages-and-explicit-visibility.md)、[Product-owned progress](../docs/decisions/provide-product-owned-check-progress.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md)、[Core facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)与[machine v4](../docs/decisions/publish-fingerprint-bound-check-record-machine-v4.md)（均 aligned）。 | [Configuration](../docs/configuration.md)、[Architecture](../docs/architecture.md) 与 [Output](../docs/output.md) 完整定义 descriptor-safe attachment validation、Core acceptance marker、canonical RunResult order、settled matrix/fingerprint 与 Check 按需采用；messages 不进入 Core/Record/machine。Plan 验收已闭合，归档仍需单独授权。 |
