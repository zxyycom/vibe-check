...
1. **Project Gate 与 package 交付：** 三个上游能力 Change、首轮 Gate build、repository hard cutover 与最小 Check/Record hard cut 均已归档；[repository hard cutover Change](archive/replace-workspace-verifier-with-project-gate/) 交付了 [gate-handoff.md](archive/replace-workspace-verifier-with-project-gate/gate-handoff.md)。Terminal messages/visibility 已完成实现与验收，typed dependency outputs 仍是同级 active Change 中的未完成输入；它完成后再冻结 package API documentation 与 native Gate authoring，最后刷新 Gate optimization evidence 并进入公开发布。完整依赖与 handoff 由 [Vibe Check package 与 Project Gate 交付导航](vibe-check-package-and-gate-delivery.md) 唯一承接。
2. **公共 Check authoring、typed dependency outputs、终态消息/可见性与日志证据边界：** 当前四态 Check、final data、Check-local Records、terminal messages 和 human visibility 已成为基础事实；declared direct dependency 读取上游 settled output 的能力仍由 typed dependency Change 负责。两者都不把 presentation 或日志变成 Core fact；相邻 log evidence Draft 只保存 durable receipt/event 的 owner 边界。
...
| [add-check-terminal-messages-and-visibility](add-check-terminal-messages-and-visibility/) | 交付 `CheckResult.messages`、`RunResult.checkMessages` 与显式 Check visibility 两个主要能力。 | [Check 终态消息与显式 visibility](../docs/decisions/allow-check-terminal-messages-and-explicit-visibility.md)、[Product-owned progress](../docs/decisions/provide-product-owned-check-progress.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md)、[Core Check/Record facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)与[machine v4](../docs/decisions/publish-fingerprint-bound-check-record-machine-v4.md)（均 aligned）。 | Stable owners、public candidate、Project Gate consumer 与 Plan 验收已闭合；Change 保持 active，直到后续任务单独授权归档。Minimal Record 已完成，typed dependency 是同级 Change而非前置。 |
...
| [add-check-terminal-messages-and-visibility](add-check-terminal-messages-and-visibility/) | 当前 Product 让需要补充提示的 Check 在 `CheckResult.messages` 附带 level/code/message，settlement 时呈现并由 `RunResult.checkMessages` 返回，同时提供 `always | attention` visibility。 | [Check 终态消息与显式 visibility](../docs/decisions/allow-check-terminal-messages-and-explicit-visibility.md)、[Product-owned progress](../docs/decisions/provide-product-owned-check-progress.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md)、[Core facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)与[machine v4](../docs/decisions/publish-fingerprint-bound-check-record-machine-v4.md)（均 aligned）。 | Stable owners close descriptor-safe attachment validation、Core acceptance marker、canonical RunResult order、settled matrix/fingerprint与Check按需采用；messages不进入Core/Record/machine。Plan 验收已闭合，归档仍需单独授权。 |
...
1. **性能策略：** Product progress、可见序号、stream ownership、effect failure isolation、terminal messages/visibility 当前均不提供 live/intermediate output；是否把 duration 变成 performance policy仍需真实消费者和独立 Decision。
