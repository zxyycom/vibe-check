...
1. **Project Gate 与 package 交付：** 三个上游能力 Change、首轮 Gate build、repository hard cutover 与最小 Check/Record hard cut 均已归档；[repository hard cutover Change](archive/replace-workspace-verifier-with-project-gate/) 交付了 [gate-handoff.md](archive/replace-workspace-verifier-with-project-gate/gate-handoff.md)。当前 typed dependency outputs 与 terminal messages/visibility 是同级 active Change，彼此不互相阻塞；两者完成后再冻结 package API documentation 与 native Gate authoring，最后刷新 Gate optimization evidence 并进入公开发布。完整依赖与 handoff 由 [Vibe Check package 与 Project Gate 交付导航](vibe-check-package-and-gate-delivery.md) 唯一承接。
2. **公共 Check authoring、typed dependency outputs、终态消息/可见性与日志证据边界：** 当前四态 Check、final data 和 Check-local Records 已成为基础事实；declared direct dependency 读取上游 settled output 的能力由 typed dependency Change 负责，terminal messages 与 human visibility 由独立 Plan 负责。两者都不把 presentation 或日志变成 Core fact；相邻 log evidence Draft 只保存 durable receipt/event 的 owner 边界。
...
| [add-check-terminal-messages-and-visibility](add-check-terminal-messages-and-visibility/) | 交付 `CheckResult.messages`、`RunResult.checkMessages` 与显式 Check visibility 两个主要能力。 | [Check 终态消息与显式 visibility](../docs/decisions/allow-check-terminal-messages-and-explicit-visibility.md)（future）；[Product-owned progress](../docs/decisions/provide-product-owned-check-progress.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md)、[Core Check/Record facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)与[machine v4](../docs/decisions/publish-fingerprint-bound-check-record-machine-v4.md)（aligned）。 | 已进入 Plan：需要补充提示的 Check 按需附带 settlement-only messages；`always | attention` visibility 参与 fingerprint，Project Gate process 只附带批准的 `command-failed` 摘要。Minimal Record 已完成，typed dependency 是同级 Change而非前置。 |
| [ship-public-package-api-documentation](ship-public-package-api-documentation/) | 为 public symbols/declarations 补完整 JSDoc，并把版本匹配的 README/API guide 作为 exact candidate material 交付。 | [版本化 package unit](../docs/decisions/release-one-versioned-npm-product-unit.md)、[Bun host](../docs/decisions/support-bun-as-the-package-host.md)、[API-only entry](../docs/decisions/use-programmatic-api-as-product-entry.md)、[`0.0.x`](../docs/decisions/keep-prestable-package-releases-on-0-0-x.md)（均 future）。 | cutover 后实施，依赖最小 Record contract、typed dependency outputs 与 terminal messages/visibility；向 Gate optimization 与 publish 交付 documentation handoff。 |
...
| [add-check-terminal-messages-and-visibility](add-check-terminal-messages-and-visibility/) | 让需要补充提示的 Check 在 `CheckResult.messages` 附带 level/code/message，settlement 时呈现并由 `RunResult.checkMessages` 返回，同时交付 `always | attention` visibility。 | [Check 终态消息与显式 visibility](../docs/decisions/allow-check-terminal-messages-and-explicit-visibility.md)（future）；[Product-owned progress](../docs/decisions/provide-product-owned-check-progress.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md)、[Core facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)与[machine v4](../docs/decisions/publish-fingerprint-bound-check-record-machine-v4.md)（aligned）。 | Plan 已固定closed attachment validation、Core acceptance marker、canonical RunResult order、settled matrix/fingerprint与Check按需采用；messages不进入Core/Record/machine。 |
...
1. **性能策略：** Product progress、可见序号、stream ownership 与 effect failure isolation 已由对应 Decision 闭合；terminal messages/visibility 已有独立 Plan，且不提供 live/intermediate output。未来是否把 duration 变成 performance policy仍需真实消费者和独立 Decision。
