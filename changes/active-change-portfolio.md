...
| [align-project-gate-with-native-check-authoring](align-project-gate-with-native-check-authoring/) | 在 cutover 后让权威 Gate catalog 组合普通 public `Check` values，把 process execution 降级为可选 helper，删除形成时数量锁。 | [普通 `Check` surface](../docs/decisions/expose-recursive-check-authoring-and-run-surface.md)、[原生对象组合](../docs/decisions/use-native-object-composition-for-check-customization.md)、[完整 Gate 顺序](../docs/decisions/complete-project-gate-before-public-package-release.md)（均 aligned）。 | 保持正式 bindings；等待 Record、typed dependency outputs、presentation 与 documentation 后，对 exact artifact 交付 `gate-optimization-handoff.md`。 |
