...
| --- | --- | --- | --- |
| [add-typed-check-dependency-outputs](add-typed-check-dependency-outputs/) | 让 declared direct dependency 通过 `dependencies.get(checkId)` 读取 settled canonical final data，再由 producing Check 的 `parseData` 恢复 typed data。 | [String getter读取direct dependency final data](../docs/decisions/read-direct-dependency-final-data-by-string.md)（unaligned）；[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md)与[Core final-data facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)（aligned）。 | Getter保持pure string与runtime direct authorization；provider-local Data generic只同步execution/parser，首版不加入Record getter，并以changed-files多consumer、declaration emit与external readback证明边界。 |
...
| --- | --- | --- | --- |
| [add-typed-check-dependency-outputs](add-typed-check-dependency-outputs/) | 让pure string dependency getter、canonical runtime data与Check-owned provider parser形成分层契约，不建立跨Check literal泛型链。 | [String getter读取direct dependency final data](../docs/decisions/read-direct-dependency-final-data-by-string.md)（unaligned）；[普通 `defineCheck`](../docs/decisions/expose-ordinary-check-values-with-define-check.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md)与[Core final-data facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)（aligned）。 | Minimal Record前置已满足；runtime按effective string IDs授权并返回两种closed failure，provider-local Data generic同步execution/parser，四态settlement移除implicit unavailable cascade，Record variant因无named consumer退出首版。 |
