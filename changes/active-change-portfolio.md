...
1. **Project Gate 与 package 交付：** 三个上游能力 Change 与首轮 Gate build 已归档；已归档的 [repository hard cutover Change](archive/replace-workspace-verifier-with-project-gate/) 交付了 [gate-handoff.md](archive/replace-workspace-verifier-with-project-gate/gate-handoff.md)。后续先实施最小 Check Record hard cut；typed dependency outputs 与 Check-associated result presentation 都以它为前置，但彼此不互相阻塞。两者完成后再冻结 package API documentation 与 native Gate authoring，最后刷新 Gate optimization evidence 并进入公开发布。完整依赖与 handoff 由 [Vibe Check package 与 Project Gate 交付导航](vibe-check-package-and-gate-delivery.md) 唯一承接。
2. **公共 Record/Check authoring、typed dependency outputs、结果呈现与日志证据边界：** 让任意 custom Check 通过 `records.report({ id }, data)` 提交自己的 canonical data，让 declared direct dependency 读取上游 settled outcome/Records 并通过 Check-owned parser 获得类型；presentation、comparison 和 domain verdict 分别留给命名 consumer 或 producing Check。相邻 Draft 继续设计 terminal/live content 与 durable log/event 的 owner 分层。
...
| --- | --- | --- | --- |
| [establish-minimal-check-record-contract](establish-minimal-check-record-contract/) | 让任意 custom/default Check 通过 `records.report({ id }, data)` 提交 canonical data，并把 Core/machine 与旧 Record 的直接消费者硬切到 `{ checkId, id, data }`。 | [Check-local Record data](../docs/decisions/report-check-owned-record-data-with-local-identities.md)、[Check-owned comparison](../docs/decisions/keep-comparison-semantics-inside-producing-checks.md)、[直接 execution 与最小 reporting](../docs/decisions/use-direct-check-execution-with-minimal-record-reporting.md)（均 future）；[Core Check/Record facts](../docs/decisions/use-core-check-and-record-facts-from-run-resolution.md)（aligned）。 | 只删除旧 Record 拥有的 comparison/reference 输入；不重构其它 execution inputs。完成后同时向 typed dependency 与 presentation 交付 minimal Record facts。 |
| [add-typed-check-dependency-outputs](add-typed-check-dependency-outputs/) | 让 declared direct dependency 读取 settled outcome/Records，并通过 Check-owned parser 获得 typed data 或 failure。 | [Typed settled dependency outputs](../docs/decisions/let-dependent-checks-read-settled-upstream-outputs.md)（future）；[Core Check/Record facts](../docs/decisions/use-core-check-and-record-facts-from-run-resolution.md)（aligned）。 | 等 minimal Record 成为当前事实后再进入 Plan；以 changed-files producer 和两个 consumers 证明类型链与 runtime boundary，不顺带清理其它 execution inputs。 |
| [add-check-associated-result-presentation](add-check-associated-result-presentation/) | 交付 Check-associated terminal presentation 与显式 Check visibility 字段，并决定首版是否包含 live/intermediate feedback。 | [Product-owned progress](../docs/decisions/provide-product-owned-check-progress.md)、[Core Check/Record facts](../docs/decisions/use-core-check-and-record-facts-from-run-resolution.md)、[machine v3](../docs/decisions/publish-fingerprint-bound-check-record-machine-v3.md)（均 aligned）。 | 等 minimal Record 成为当前事实后再进入 Plan；它与 typed dependency 是同级后续 Change，不互相阻塞。Visibility 只控制 human direct display。 |
| [ship-public-package-api-documentation](ship-public-package-api-documentation/) | 为 public symbols/declarations 补完整 JSDoc，并把版本匹配的 README/API guide 作为 exact candidate material 交付。 | [版本化 package unit](../docs/decisions/release-one-versioned-npm-product-unit.md)、[Bun host](../docs/decisions/support-bun-as-the-package-host.md)、[API-only entry](../docs/decisions/use-programmatic-api-as-product-entry.md)、[`0.0.x`](../docs/decisions/keep-prestable-package-releases-on-0-0-x.md)（均 future）。 | cutover 后实施，依赖最小 Record contract、typed dependency outputs 与首版 result presentation；向 Gate optimization 与 publish 交付 documentation handoff。 |
| [align-project-gate-with-native-check-authoring](align-project-gate-with-native-check-authoring/) | 在 cutover 后从独立质量事实重建权威 Gate：直接组合 ordinary Checks/typed capabilities，CLI lifecycle 独立审计，只为真实外部边界保留 process，并删除迁移重复项与数量锁。 | [caller-runtime Check execution](../docs/decisions/execute-check-functions-in-caller-runtime.md)、[普通 `Check` surface](../docs/decisions/expose-recursive-check-authoring-and-run-surface.md)、[完整 Gate 顺序](../docs/decisions/complete-project-gate-before-public-package-release.md)（均 aligned）。 | 先闭合 required/full 与 foundation package evidence 两项 inventory；保持正式 bindings，等待 Record、typed dependency outputs、presentation 与 documentation 后，对 exact artifact 交付 `gate-optimization-handoff.md`。 |
...
这一组 active Change 都建立在普通 Check、直接 execution 与 `checks` / `records` Core facts 之上。它们不自动授权公共 API 变化，不把 Record presence 解释为 Check verdict，也不把人读 progress 或项目 transcript 误作结构化事件协议。

| Change | 此 Change 的核心目的 | 直接 Decision 输入 | 依赖、恢复或未决边界 |
| --- | --- | --- | --- |
| [establish-minimal-check-record-contract](establish-minimal-check-record-contract/) | 把 Product-wide Record 共同分母限制为 Check-local `id`、custom canonical `data` 和 reporter scope 提供的 `checkId`；typed readback 使用 optional Check-owned parser。 | [Check-local Record data](../docs/decisions/report-check-owned-record-data-with-local-identities.md)、[Check-owned comparison](../docs/decisions/keep-comparison-semantics-inside-producing-checks.md)、[直接 execution 与最小 reporting](../docs/decisions/use-direct-check-execution-with-minimal-record-reporting.md)（均 future）；[普通 `defineCheck`](../docs/decisions/expose-ordinary-check-values-with-define-check.md)、[Core Check/Record facts](../docs/decisions/use-core-check-and-record-facts-from-run-resolution.md)（aligned）。 | 只迁移旧 Record contract 的直接消费者；其它 execution inputs 保持现状，等 typed capability 已实施且存在真实 consumer 后再另立范围。 |
| [add-typed-check-dependency-outputs](add-typed-check-dependency-outputs/) | 让 dependency getter、parser descriptor 与 typed result 形成可推导的 TypeScript 关系。 | [Typed settled dependency outputs](../docs/decisions/let-dependent-checks-read-settled-upstream-outputs.md)（future）；[普通 `defineCheck`](../docs/decisions/expose-ordinary-check-values-with-define-check.md)与[Core Check/Record facts](../docs/decisions/use-core-check-and-record-facts-from-run-resolution.md)（aligned）。 | Minimal Record 是 implementation 前置；领先方向是 Check-value `dependsOn`，getter 负责选择和 failure，parser 只转换已选 data。 |
| [add-check-associated-result-presentation](add-check-associated-result-presentation/) | 交付 terminal Record projection 与显式 `always` / `problems-only` Check visibility。 | [Product-owned progress](../docs/decisions/provide-product-owned-check-progress.md)、[Core Check/Record facts](../docs/decisions/use-core-check-and-record-facts-from-run-resolution.md)、[machine v3](../docs/decisions/publish-fingerprint-bound-check-record-machine-v3.md)（均 aligned）。 | Minimal Record 是 implementation 前置；typed dependency 只是 consumer。Visibility 不抑制 facts/events；若采用 live feedback，必须先演进 progress Decision。 |
