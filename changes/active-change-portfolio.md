...
本文件只拥有跨 Change 的阅读路径、分组和关系摘要，不拥有任何 Change 的动态 stage、任务完成、实现事实或长期方向。下文提到的 `add-typed-check-dependency-outputs` “20/20、active、未归档”只摘要其最近已记录状态；必须以该 Change 的 `tasks.md` 和 `bun run change-plan -- list changes` 核对，不能把 `active` 推断为仍有实施任务。
...
| --- | --- | --- | --- |
| [add-typed-check-dependency-outputs](add-typed-check-dependency-outputs/) | 已完成：declared direct dependency 通过 `dependencies.get(checkId)` 读取 settled canonical final data，再由 producing Check 的 `parseData` 恢复 typed data。 | [String getter 读取 direct dependency final data](../docs/decisions/read-direct-dependency-final-data-by-string.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md) 与 [Core final-data facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)（均 `aligned`）。 | 任务记录为 20/20，required/full Gate 已闭合；Change 只因未获归档授权保持 active。首版没有 Record getter、跨 Check generic 或新 runtime root。 |
...
| --- | --- | --- | --- |
| [add-typed-check-dependency-outputs](add-typed-check-dependency-outputs/) | 已用 pure string dependency getter、canonical runtime data 与 Check-owned provider parser 形成分层契约，没有建立跨 Check literal 泛型链。 | [String getter 读取 direct dependency final data](../docs/decisions/read-direct-dependency-final-data-by-string.md)、[普通 `defineCheck`](../docs/decisions/expose-ordinary-check-values-with-define-check.md)、[four-state final data](../docs/decisions/use-four-state-check-results-with-final-data.md) 与 [Core final-data facts](../docs/decisions/use-core-check-record-facts-with-final-data.md)（均 `aligned`）。 | Runtime 按 effective string IDs 授权并返回两种 closed failure，provider-local Data 同步 execution/parser，四态 settlement 移除 implicit unavailable cascade；Record variant 因无 named consumer 未进入首版。 |
