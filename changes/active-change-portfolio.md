...
1. **Project Gate 与 package 交付：** 三个上游能力 Change 与首轮 Gate build 已归档；当前先完成 repository hard cutover，再在唯一正式 Gate 下补全 typed Record、首版 Check-associated result presentation、package API documentation 与 native Check authoring，最后刷新 Gate optimization evidence 并进入公开发布。完整依赖与 handoff 由 [Vibe Check package 与 Project Gate 交付导航](vibe-check-package-and-gate-delivery.md) 唯一承接。
...
| --- | --- | --- | --- |
| [replace-workspace-verifier-with-project-gate](replace-workspace-verifier-with-project-gate/) | 消费归档 readiness 并在当前 revision 重跑对照，将 Gate 切换为唯一仓库门禁并退役旧 verifier。 | [完整 Gate 顺序](../docs/decisions/complete-project-gate-before-public-package-release.md)（future）。 | 当前第一优先级；交付 `gate-handoff.md` 的 binding/legacy-retirement 事实。正式 root/CI 运行无 disabled tags 的 required/full。 |
...
| [align-project-gate-with-native-check-authoring](align-project-gate-with-native-check-authoring/) | 在 cutover 后让权威 Gate catalog 组合普通 public `Check` values，把 process execution 降级为可选 helper，删除形成时数量锁。 | [普通 `Check` surface](../docs/decisions/expose-recursive-check-authoring-and-run-surface.md)、[原生对象组合](../docs/decisions/use-native-object-composition-for-check-customization.md)（aligned）；[完整 Gate 顺序](../docs/decisions/complete-project-gate-before-public-package-release.md)（future）。 | 保持正式 bindings，最终对 documentation-complete artifact 交付 `gate-optimization-handoff.md`。 |
