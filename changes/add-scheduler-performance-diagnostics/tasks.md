...
- [ ] 0.2 审阅 `require-passed-dependencies-and-observe-outcomes`、`add-invocation-fail-fast-policy` 与 `add-named-resource-capacity` 的实际stage和最终Scheduler影响，确定实施顺序；同步本Plan的graph-ready、await reason与effective capacity定义。若任一前置模型尚未稳定，暂停受影响的Implementation任务，不编码短命中间模型。
...
- [ ] 1.2 从现有execution state的admit、await、settlement、scope activation/release和terminal transition派生Task chronology、slot/capacity积分、max running、blocked-admission reason intervals、top-three admission delays、last settlement与completion tail，不建立第二套pending/running真相。
...
- [ ] 2.1 使用具名scripted-clock fixtures运行最窄task-scheduler tests，精确证明空图、单Task、并行Task、依赖释放、mutex/root/scope/reservation等待、priority、blocked settlement、running/cancellation drain、相同delay排序、最大并发、利用率分母与tail公式。
