...
HTTP(S)、protocol-relative、`mailto:`、UNC 与带 remote authority 的 `file:` form 只被分类后停止：不做 DNS、HTTP、TLS、redirect、retry 或 external reachability verdict，也不把 raw request material 交给其它 Check。只有无 remote authority 的 supported local `file:` form 才可能进入本机 target option gate。

## Scope

### Intended Change

1. 新增 `markdownLinkValidation` ordinary default value、closed options runtime validation 与 public export。候选 grammar 以 `requireExistingTargets`、`validateSameDocumentAnchors`、`validateCrossDocumentAnchors`、project-root 外 target 的互斥 `ignore | report | validate` mode，以及默认关闭的 `requireNonEmptyDirectories` 表达行为；L5 固定字段名、defaults、normal reasons 与完整 public grammar。
2. Link Check 自己拥有一个最小、package-private Markdown parser adapter。已选 architecture 是低层 `mdast-util-from-markdown`、所需 micromark/mdast GFM 与 front matter extensions，以及 `github-slugger`；它只投影本 Check 使用的 occurrence、definition、heading/anchor input、source ranges 和 controlled parse failure，并封装 dependency AST。精确 production semver 与最小 extension set 由 L4 的 dependency/license/Bun evidence 固定；它不依赖 `markdownStructureValidation` 注册、执行或通过，也不建立 Check-to-Check handoff、parse cache 或 public Markdown model。
...
4. 本地 resolution 按安全顺序处理：先受控 decode/classify，再以 source directory 为 base 做 lexical normalization。source eligibility 始终是 global exact scope；root 内但 source scope 外的 direct target 可被 bounded 检查，且不成为新 source input。root 外、absolute 或 lexical/realpath escape 只有 `validate` mode 才允许对 source 明确指向的 target 做 `lstat`、`realpath`、目录 entry 或 Markdown content read；`report` mode 只形成 safe boundary finding，`ignore` mode 不读取也不汇报。没有递归 discovery。cross-document anchor 只能在可读取的 Markdown regular file 上解析，directory target 不进行 anchor read。
...
- 本地 file、directory、source-scope 外、absolute、lexical escape 与 symlink escape 的行为符合 L3 的明确 contract；root 外 mode 的 `ignore | report | validate` fixture 证明不读取、只汇报或 direct bounded validation 的区别，directory target 不进行 anchor read。
