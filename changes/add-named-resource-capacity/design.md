...
- `admissionPriority`只在现有eligible层级中排序，不能绕过dependency、mutex、scope capacity或 Scheduler hard guard。
- 无状态 policy 每轮从完整 graph、candidate、capacity 与 runtime facts重算；新增resource capacity不得把reservation、抢占状态或任何特定防饥饿算法写入Scheduler Core。
...
5. priority只在无状态 policy 的capacity-eligible Task比较中排序。resource shortage下如何以完整当前事实保证有限进展、以及大weight Task饥饿是否是真实需要，必须在Plan前用纯decision tests闭合；不得预置Core reservation。
...
- Scheduler readiness、capacity hard guard、blocker summary、diagnostic rendering、cancellation和settlement release需要共同修改；算法仍应保持单一pending集合、无状态 policy 与确定性。
...
- 特定资源分配算法若为大weight Task预留capacity可能延迟许多小Task；若不预留则可能使大Task饥饿。首版不得把任一算法预置为Scheduler reservation；真实消费者决定是否需要 policy-owned state。
...
- 大weight Task是否需要超出无状态有限进展模型的算法；若需要，最小的 policy-owned state 与 admission priority 如何组合？
