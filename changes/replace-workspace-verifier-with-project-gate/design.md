...
- 将仓库 root scripts、CI/workflow 与文档切换到同一个 Project Gate implementation；名称沿用或调整都只是普通接线，保留的 command name 只能薄转发。
- 让正式 repository/CI bindings 只调用无 disabled tags 的 required/full；不增加 CI 环境检测，也不移除 local adapter 的显式 partial 能力。
- repo-wide 定位旧 workspace verifier implementation、调用引用、测试和失效说明，删除已无调用者的精确目标并保留清晰的 VCS rollback 边界。
- 从完成接线后的实际 root/CI bindings 重跑无 disabled tags 的 required/full，并写出 <code>gate-handoff.md</code> 给 release Change。
...
正式 root scripts、CI 和文档均指向 Project Gate。command spelling 不承载产品语义；无论沿用还是调整名称，所有保留的调用都必须到达同一 implementation，不能继续维护第二套 command tree、profile selection 或 result interpretation。

### 3. 删除必须可核对且可回退

先用 repo-wide reference audit 定位旧 verifier 的定义、scripts、package commands、CI/workflow、文档和测试引用；这项审计产生当前工作树的机械删除范围，不需要用户预先决定一份固定文件清单。切换后的最窄验证通过后，只删除已无调用者的精确目标。回退是恢复该 Change 的 source/binding references，不是让新旧实现长期并行成为两个门禁真相。
...
正式 repository root scripts 与 CI/workflow 只绑定无 disabled tags 的 required/full，因此不能通过正式配置关闭某些 Check。Gate adapter 不读取 ambient `CI` 标记，也不因同一个 local partial command 恰好在 CI host 上运行就行为性拒绝；contract review 和 workflow/root-script evidence 负责证明正式调用没有传 disabled tags。
...
无。command name 不是语义决策；正式 repository/CI contract 固定为无 disabled tags 的 required/full；cutover 验收从完成接线后的实际 root/CI bindings 运行这两个 profile；legacy 删除目标由 repo-wide reference audit 机械确定。
