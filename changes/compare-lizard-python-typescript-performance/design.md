...
本设计以三个严格标注的测量层而非单一语言结论，兑现开发/验证专用的 Python/Lizard 与 TypeScript analyzer 性能比较。
...
- 本 Change 尚未取得任何 comparative sample；Python 更快、TypeScript 更快或无差异均是待测假设，不能从 implementation language、old observation 或 archive 推导。

## Goals / Non-Goals

**Goals**

- 为三种不同问题提供可重复且不可混淆的 measurement：迁移后用户可感知总成本、固定 semantic analyzer cost、以及 current Product stage attribution。
...
- 不优化 analyzer、替换 runtime、改变 reader semantics、建立 backend abstraction、公开 façade、增加 Product CLI/`bin`、恢复 Python/Lizard/CSV/subprocess/fallback，或为 Worker 建 pool/cache。
...
1. **Three named workloads with a common evidence envelope.**
   ...
   - **C — current Product decomposition:** for the same current fixture, use current-internal measurement hooks only to emit read/decode, Worker construction/message transfer, adapter/port analysis and total durations without altering timing behavior. Verify that the reported request/result digest agrees with the normal current Product invocation. It attributes current cost; no individual component is advertised as historical equivalence.
...
- Which pinned Python/Lizard 1.24 installation mechanism can be reproduced on the initial Linux x64 host without adding it to the Product, package candidate or default environment setup? This must be resolved in Readiness before implementation.
- Does the selected host provide a validated supervisor for process-tree CPU and peak RSS, or must A publish parent/child-scoped fields and an explicit non-comparable aggregate? The implementation cannot claim a combined RSS until this is answered.
- What exact corpus breadth is sufficiently representative while remaining supported by both historical 1.23 Product and current 1.24 Product? Readiness must record the intersection and any intentionally excluded reader/version differences.
