...
本 Plan 的任务按“长期决定与 current contract → tree/option implementation → dogfood 与全链验证”顺序执行。已经成立且未受 API 修订影响的证据继续保留；原先以 nested spread 为验收依据的任务重新打开，直到 descriptor-owned adjustment contract 完整实现并验证。
...
- [x] 0.2 使用 `decision-records`：先运行 `bun run decisions:list`，建立修订 nested-spread 选择的 field-aware adjustment decision，并核对 public built-in descriptor / Check tree authoring、typed option owner 与 Check-to-Task mutex handoff；运行 `bun run decisions:check`。
...
- [x] 1.1 扩展 current public-contract source 和 consumer comparison：拥有 `duplicateDetection`、`fileMetrics`、`functionMetrics` 的 exported-value names、descriptor `.replace()` / `.append()` member names、必要 types 和 callable/non-callable inventory；不建立第二份 public-name list。
- [x] 1.2 在各 built-in owner 形成 frozen non-callable descriptor：保留 stable Check/Record metadata 和 complete typed defaults，增加 field-aware `.replace()` / `.append()`、immutable chaining 与 private pure-data materialization，并拒绝对 built-in identity/private binding 的伪造或改写。
...
- [x] 1.8 更新 repository Project Definition、Project Run dogfood aliases和 fixtures，使用直接导入的 built-ins、tree grouping与 descriptor-owned adjustment methods；不得通过 scripts 恢复参数解析、config discovery或另一个 config source。
- [x] 1.9 更新 Configuration、current-contract、quality/scanner、Task/Check/Record、output/testing/navigation docs与 examples；使用 `ai-ready-docs` 审核，使 adjustment、inheritance、default concurrency、private boundary和 non-goals可从 owner 文本独立恢复。
- [x] 1.10 更新 `establish-api-only-npm-product-boundary` 的 proposal/design/tasks：把本 Change 的 value-owned methods 纳入 descriptor declarations 和 exact-tarball consumer evidence；不在该 Change 实现 package projection。

## Verification

- [x] 2.1 运行 Check tree authoring/validation/normalization tests：built-in direct/adjusted/spread use、custom/built-in mixed nesting、selection-by-presence、group flattening、unique namespace、unknown/self/cyclic group dependencies、inherited append-dedupe与 stable diagnostics。
- [x] 2.2 运行 built-in options migration tests：descriptor defaults、field-aware replacement、scheduling append、immutable chaining、project-wide quality inputs、scanner-private field exclusion、metadata canonicality与 prior quality result semantics。
...
- [x] 2.4 运行 Project Definition/Run Controls、current contract、catalog/Check/Record/policy/fingerprint、built-in runtime、dogfood Project Run 和 output contract的最窄目标 tests；验证 authoring methods、groups、functions、private bindings与 Task values不进入 declarative output。
