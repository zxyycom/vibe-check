...
- **依赖 / 非依赖**：本 Change 不语义依赖当前 duration/provider lifecycle seam，也不依赖其 public contract；可服务现有 custom callback。与包含该 seam 的稳定提交推荐串行/继承基线仅因共享 owner。算法 Change 可复用 private core/test harness，却不依赖 simulation public API；custom lifecycle Draft 也不与本 Change 合并。
