...
[`licenses/lizard-1.23.0-provenance.json`](licenses/lizard-1.23.0-provenance.json). That inventory is
...
- Fixed tag and revision: `1.23.0` / `06284ec87c1966fee4ddbf3f068ccf89b987b0f8`
- The translated language readers, shared reader components, registry, and extension protocol retain
  ...
  [`licenses/Lizard-1.23.0-MIT.txt`](licenses/Lizard-1.23.0-MIT.txt).
- Translated ranges from `lizard.py` retain that file's Apache-2.0 terms. The complete Apache-2.0 text
  is [`licenses/Lizard-1.23.0-lizard.py-Apache-2.0.txt`](licenses/Lizard-1.23.0-lizard.py-Apache-2.0.txt).
...
The inventory also records exactly 19 Lizard concrete extension bodies and two extension-only support
modules as `deferred-extension-body`. They are not shipped as translated source or runtime modules, are
not registered by default, and the recorded paths must not be interpreted as included implementation.
