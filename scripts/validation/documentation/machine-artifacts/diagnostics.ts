...
export function failure(
  ...
  diagnostic: Omit<DocsMachineValidationDiagnostic, "logicalArtifact" | "path">
): DocsMachineValidationFailure {
  return {
    diagnostic: {
      ...diagnostic,
...
export function setFailure(
  artifactRoot: string,
  logicalArtifact: string,
  diagnostic: Omit<DocsMachineValidationDiagnostic, "category" | "logicalArtifact" | "path">
): DocsMachineValidationFailure {
...
export function artifactPath(artifactRoot: string, logicalArtifact: string): string {
  return artifactRoot.endsWith("/")
    ? `${artifactRoot}${logicalArtifact}`
    : `${artifactRoot}/${logicalArtifact}`;
}

export function formatDiagnostic(diagnostic: DocsMachineValidationDiagnostic): string {
  const locations = [
