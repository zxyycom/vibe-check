    ...
    .map((check) => ({
      ...
      recordTypes: check.recordTypes
        .map((recordType) => ({
          ...
          policy: {
            operands: [...(recordType.policy?.operands ?? [])].sort((left, right) =>
              compareText(left.operandId, right.operandId)
            ),
...
export function recordIdentity(record: RecordShape, recordType: RecordTypeShape): string {
  const identityFields = Object.fromEntries(
    recordType.identityFields.map((fieldId) => [fieldId, record.fields[fieldId]])
  );
...
export function referenceEvidenceKey(value: { checkId: string; referenceName: string }): string {
