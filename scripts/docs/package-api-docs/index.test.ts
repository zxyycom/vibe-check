...
import { runPackageApiDocumentationCli } from "./index.ts";
...
describe("package API documentation CLI", () => {
  it("writes expected projections and detects stale output through --check", () => {
    ...
    try {
      ...
      const jsdocPath = join(fixtureRoot, "src/product/definition/custom-check.ts");
      writeFileSync(
        jsdocPath,
      ...
      assert.match(staleJSDoc.diagnostics[0] ?? "", /src\/product\/definition\/custom-check\.ts/);
      assert.equal(
        runPackageApiDocumentationCli(["--write"], { repositoryRoot: fixtureRoot }).exitCode,
        0
      );
