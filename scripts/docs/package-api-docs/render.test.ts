...
describe("package API documentation renderer", () => {
  it("projects every registry region to its declared README and JSDoc targets without changing payload bytes", () => {
    ...
    try {
      ...
      assert.equal(
        ...
        join(fixtureRoot, "src/product/definition/custom-check.ts")
      );
  ...
  it("replaces generated JSDoc tails and rejects malformed source regions", () => {
    ...
    try {
      const sourcePath = join(staleJSDocFixture, "src/product/definition/custom-check.ts");
      writeFileSync(
        sourcePath,
        [
          "/**",
    ...
    try {
      const sourcePath = join(removedJSDocTargetFixture, "src/product/definition/custom-check.ts");
...
function createDocumentationFixture(): string {
  ...
  const sourcePath = join(fixtureRoot, "src/product/definition/custom-check.ts");
...
function writeProductSource(fixtureRoot: string, name: string, content: string): void {
  const path = join(fixtureRoot, "src/product", name);
