...
import { isNonArrayRecord, isStringArray } from "../../tools/foundation/src/index.ts";
...
import { createNativeOperationCheck } from "./native-check.ts";
import { createProjectGateDefinition, createProjectGateEntries } from "./project-definition.ts";
import { writeProcessTranscript } from "./process-check.ts";
...
} from "./test-evidence-rule-tests-check.ts";
...
describe("Project Gate Definition", () => {
  it("projects ordinary Check entries without a command catalog or policy", () => {
    ...
    assert.match(scanOnlyEntry, /scripts\/quality\/scan\.ts$/);
